     ...
     1      x(10 000),whts(10 000),rlams(10 000),w(1000 000),
...
c 
        par1(i)=i**2
cccc        par1(i)=par1(i)/(1+par1(i)) * 10
c 
cccc        par1(i)=par1(i)**2
  
  
        par1(i)=par1(i)/(1+par1(i)**2) * 10
...
c 
        ifab=1
c 
        call allsvbld(ier,a,b,k,fun2,par1,par2,nfuns,eps,
     1    ab,lab,ifab,nn,x,whts,n,rlams,lrlams,w,lw,ncols,
     2      rints,lused,igraph)
...
c 
     ...
     2      rints,lused,igraph)
        ...
        call prin2('after allsvbld, x=*',x,n)
        call prin2('after allsvbld, ab=*',ab,nn*2)
c 
...
cccc        call prin2('for this i, ftest=*',ftest,ntest)
  
 1700 format('      i=',i4)
        write(21,1700) i
 1800 format(6x,e23.16,6x,e23.16)
        write(21,1800) (ftest(j), j=1,ntest)
...
c 
c 
cccc        subroutine fun2(x,i,par1,par2,par3,f)
        subroutine fun2(x,i,par1,par2,f)
        implicit real *8 (a-h,o-z)
cccc        dimension par1(1),par2(1),par3(1)
        save
...
c 
c 
        subroutine allsvbld(ier,a,b,k,fun,par1,par2,
...
c 
        lused=iww+lww
c 
        call allffuns(jer,x,whts,n,nfuns,fun,par1,par2,
     1      eps,w(icoefs),lencoefs,ncols,rlams,nn,k,w(iu),w(iv),
     2      w(iww),rints,lused2,lrlams,igraph)
c 
        lused=lused+lused2
c 
        if(jer .ne. 0) ier=8
        if(jer .ne. 0) return
c 
        call mvecmove(w(icoefs),w,ncols*n)
c 
        return
        end
c 
...
c 
        call mvecsvd(ier,allsvmat,fun,par1,par2,x,ww,n,
     1    nfuns,eps,ncols,sss,lrlams,
     2      fs,lfs,lused)
c 
        if(ier .ne. 0) return
...
c 
        ...
        implicit real *8 (a-h,o-z)
        save
        dimension coefs(nn*k,1),ab(2,1),pjcoefs1(300),pjcoefs2(300)
        data intold/-10/,ifcalled/0/
c 
...
c        allsvbld (see) as its input; it has no function as a
...
c 
        jj=(intnum-1)*k+1
c 
cccc        call legeFDER(t,VAL,der,coefs(jj,i),k-1)
c 
        ninit=290
        if(ifcalled .eq. 1) ninit=0
        ifcalled=1
c 
        call legeFDE2(t,VAL,der,coefs(jj,i),k-1,

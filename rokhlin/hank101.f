        ...
        complex *16 ima,z,h0,h1
        dimension zrea(2),derrs0(20 000),
     1      derrs1(20 000),xs(20 000),ys(20000)
        data ima/(0.0d0,1.0d0)/
        equivalence (z,zrea(1))
c 
        complex *32 qz,qh0,qh1,qw(10000),qd,qerr0,qerr1
c 
        call prini(6,13)
C 
...
c 
c 
c        test the asymptotic expansions
c        of h0, h1
c 
           if(2 .ne. 3) goto 2000
         call hank101a(z,h0,h1)
c 
         call prin2('after hank101a, h0=*',h0,2)
         call prin2('after hank101a, h1=*',h1,2)
c 
c       calculate them in extended precision
c 
        qd=zrea(1)
        qz=qd
        qz=z
        ioption=0
        m=33
         call prinq('before h01com, qz=*',qz,2)
        call foolcoi(ier)
        call h01com(qz,qh0,qh1,qw,ioption,m)
         call prinq('after h01com, qh0=*',qh0,2)
         call prinq('after h01com, qh1=*',qh1,2)
        call prinq('h0-qh0=*',h0-qh0,2)
        call prinq('h1-qh1=*',h1-qh1,2)
c 
          stop
 2000 continue
c 
c        test the intermediate regime
c 
          call prin2('testing first intermediate regime, z=*',z,2)
         h0=0
         h1=0
cccc        call hank101u(z,ier,h0,h1)
cccc        call hank101r(z,ier,h0,h1)
  
         call hank101(z,h0,h1)
c 
         call prin2('after hank101, h0=*',h0,2)
         call prin2('after hank101, h1=*',h1,2)
c 
c       calculate them in extended precision
c 
        qd=zrea(1)
        ...
        ioption=0
        m=33
cccc         call prinq('before h01com, qz=*',qz,2)
        call foolcoi(ier)
        call h01com(qz,qh0,qh1,qw,ioption,m)
cccc         call prinq('after h01com, qh0=*',qh0,2)
cccc         call prinq('after h01com, qh1=*',qh1,2)
        call prinq('h0-qh0=*',h0-qh0,2)
        call prinq('h1-qh1=*',h1-qh1,2)
c 
c       perform massive testing of the subroutine
c 
cccc        a=2
cccc        ntest=10
        h=2*a/ntest
...
c 
         qerr0=qh0-h0
         qerr1=qh1-h1
         call prinq('qerr0=*',qerr0,2)
         call prinq('qerr1=*',qerr1,2)
         derrs0(ij)=cqabs(qerr0)/cdabs(h0)
         derrs1(ij)=cqabs(qerr1)/cdabs(h1)
 2200 continue
 2400 continue
          call prin2('xs=*',xs,ntest**2)
          call prin2('ys=*',ys,ntest**2)
          call prin2('derrs0=*',derrs0,ntest**2)
          call prin2('derrs1=*',derrs1,ntest**2)
c 
c         . . . find the maxima
c 
        dmax0=0
        dmax1=0
        do 2600 i=1,ntest**2
        if(derrs0(i) .gt. dmax0) dmax0=derrs0(i)
        if(derrs1(i) .gt. dmax1) dmax1=derrs1(i)
 2600 continue
        call prin2('dmax0=*',dmax0,1)
        call prin2('dmax1=*',dmax1,1)
  
  
c 
c        finally, plot the grid on which the testing
c        has been performed
c 
        iw=21
        call RSPLOT(Xs,Ys,Ntest**2,IW,'the testing grid*')
...
c 
c 
        subroutine h01com(x,h0,h1,w,ioption,m)
        ...
        data ima/(0.0d0,1.0d0)/
c 
c        if the user has specified that the series be used
c        - use the series
c 
        iwhich=ioption
        if(iwhich .ne. 0) goto 2000
        rd=x*qconjg(x)
        iwhich=1
        if(rd .gt. 1225) iwhich=2
 2000 continue
c 
c       if appropriate - use the series
c 
        if(iwhich .eq. 2) goto 3000
c 
c       calculate the j-functions
c 
         eps=1.0d-30
        call jbq(x,eps,w(2),n)
ccc           call prinq('in h01com after jbq, w=*',w,n)
c 
c       calculate the j-function
c 
        call ybq(x,y0,w(2),w(1002),y1)
c 
        h0=w(1)+ima*y0
        h1=w(2)+ima*y1
c 
...
c 
        ...
        real *16 eps,done,pi,gamma
cccc        data gamma/0.5772156649015328606d+00/
        data ifcalled/0/
c 
c       calculate the j-functions
c 
...
c 
        ...
        pi=qatan(done)*4
        i0=0
cccc 1150 format('    pi= ',E23.17)
cccc        write(6,1150) pi
cccc        write(13,1150) pi
c 
...
c 
        ...
        y0=y0-dd
c 
cccc         call prinq('inside ybd, y0=*',y0,2)
c 
...
c 
        ...
        dd=1.0d50
c 
cccc          call prinq('entered jbq, x=*',x,2)
        do 1200 i=1,100000
         nn=i
        fjs(i+1)=( (2*i*done)/x)*fjs(i)-fjs(i-1)
        if( cqabs(fjs(i)) .ge. dd) goto 1400
 1200 continue
 1400 continue
...
c 
c 
        subroutine getgam(gamma)
        implicit real *16 (a-h,o-z)
c 
c        calculate ln(gamma(10000))'
c 
        save
        n=10000
        z=n
c 
        ...
        psi=qlog(z)-done/2/z-done/12/z**2+done/120/z**4-done/252/z**6
c 
c        now, calculate gamma
c 
        dd=0
        do 1200 i=1,n-1
        dd=dd+done/i
 1200 continue
        gamma=dd-psi
        call prinq('in getgam, gamma=*',gamma,1)
        return
        end
...
c 
 ...
 2200 continue
cccc         call prinq('factin as created*',factin(0),55)
        return
        end
        ...
        subroutine foolcexp(z,extexp)
        implicit real *16 (a-h,o-z)
        save
        dimension z(1),extexp(1)
c 
        d=qexp(z(1))
        call foolcosi(z(2),extcos,extsin)
        extexp(1)=d*extcos
        extexp(2)=d*extsin
        return
        end
...
c 
c 
        subroutine asymp(nn,z,m,h0,h1)
        implicit real *16 (a-h,o-z)
        save
        dimension fact(0:100),p(100),q(100),
     1      psizes(100),qsizes(100),p1(100),q1(100)
        complex *32 z,h0,pp,qq,cd,z8,ima,h1,cdd
        data ima/(0.0d0,1.0d0)/,ifinit/0/
c 
c        construct the factorials
c 
        if(ifinit .eq. 1) goto 1700
        done=1
        two=2
        four=4
        zero=0
        ...
        fact(i)=fact(i-1)*i
 1200 continue
cccc        call prinq('factorials are*',fact,60)
c 
c        evaluate the coefficients of polynomial p
c 
        p(1)=1
        ss=-1
        do 1400 i=2,nn
        j=i*4-6
cccc        p(i)=p(i-1)*(j-1)*(j+1)/fact(j)
        p(i)=p(i-1)*(j-1)**2*(j+1)**2
c 
        p(i-1)=p(i-1)/fact((i-2)*2)
        ss=-ss
        p(i-1)=ss*p(i-1)
 1400 continue
cccc        call prinq('p=*',p,nn)
c 
c       construct the coefficients of q
c 
        q(1)=-1
        ss=-1
cccc        ss=1
        do 1600 i=2,nn
        j=i*4-4
        q(i)=q(i-1)*(j-1)**2*(j+1)**2
        q(i-1)=q(i-1)/fact(2*i-3)
        ss=-ss
        q(i-1)=q(i-1)*ss
 1600 continue
cccc        call prinq('q=*',q,nn)
c 
c        evaluate the coefficients of polynomial p1
c 
        p1(1)=1
        ss=-1
        mu=4
        do 4400 i=2,nn
        j=i*4-6
        p1(i)=p1(i-1)*(mu-(j-1)**2)*(mu-(j+1)**2)
c 
        p1(i-1)=p1(i-1)/fact((i-2)*2)
        ss=-ss
        p1(i-1)=ss*p1(i-1)
 4400 continue
cccc        call prinq('p1=*',p1,nn)
c 
c       construct the coefficients of q
c 
        q1(1)=3
...
cccc        ss=1
        do 4600 i=2,nn
        j=i*4-4
        q1(i)=q1(i-1)*(mu-(j-1)**2)*(mu-(j+1)**2)
        q1(i-1)=q1(i-1)/fact(2*i-3)
        ss=-ss
        q1(i-1)=q1(i-1)*ss
 4600 continue
cccc        call prinq('q1=*',q1,nn)
...
c 
        ...
        do 1800 i=1,m
        pp=pp+p(i)*cd
         psizes(i)=cqabs(p(i)*cd)
        cd=cd*z8**2
cccc         call prinf('i=*',i,1)
 1800 continue
cccc         call prinq('and pp=*',pp,2)
cccc         call prinq('and psizes are *',psizes,m)
c 
        cd=z8
        qq=zero
        do 2000 i=1,m
        qq=qq+q(i)*cd
        qsizes(i)=cqabs(q(i)*cd)
        cd=cd*z8**2
cccc         call prinf('i=*',i,1)
cccc         call prinq('and cd=*',cd,2)
 2000 continue
cccc         call prinq('and qq=*',qq,2)
cccc         call prinq('and qsizes are *',qsizes,m)
c 
        h0=pp+ima*qq
  
  
        cd=ima*(z-pi/four)
        call foolcexp(cd,cdd)
        h0=cqsqrt(two/pi/z)*cdd * h0
  
  
cccc        h0=cqsqrt(two/pi/z)*cqexp(ima*(z-pi/four)) * h0
c 
c        finally, evaluate h1(z)
c 
        z8=z*eight
        z8=done/z8
        pp=zero
...
c 
        ...
        h1=-cqsqrt(two/pi/z)*cdd * h1*ima
  
ccccc        h1=-cqsqrt(two/pi/z)*cqexp(ima*(z-pi/four)) * h1*ima
cccc         call prinq('and h1 as computed*',h1,2)
        return
        end
c 
c 

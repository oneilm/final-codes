        ...
        dimension w(3 000 000),rlams(10 000),ipivots(10 000),
     1      a2(3 000 000),u(3 000 000)
c 
        external matfun2
c 
        call prini(6,13)
...
c 
        ...
        lenrlams=10 000
        ifsvd=1
        call mvecpsvd(ier,matfun2,par1,par2,par3,par4,par5,n,
     1    nvects,ifsvd,eps,ncols,rlams,lenrlams,ipivots,
     2    w,lenw,lused)
c 
        call prin2('after mvecpsvd, rlams=*',rlams,ncols)
        call prinf('after mvecpsvd, lused=*',lused,1)
        call prinf('after mvecpsvd, ipivots=*',ipivots,ncols)
  
c 
c       now, compute the SVD explicitly
c 
        call explic(a2,n,nvects,eps,u)
  
c 
        call diffcomp(w,u,n,ncols,rlams)
...
c 
c 
        subroutine explic(a,n,nvects,eps,u)
        implicit real *8 (a-h,o-z)
        save
        dimension a(n,nvects),w(3 000 000),s(10 000),
     1      u(1),v(3 000 000)
c 
...
c        call qrdpivot(a,n,nvects,ncols,eps,w,s)
  
  
        lw=3 000 000
        call svdpivot(ier,a,n,nvects,u,v,s,ncols,eps,
     1      w,lw,ltot)
...
c 
c 
        subroutine mvecpsvd(ier,matfun,par1,par2,par3,par4,par5,
     1    n,nvects,ifsvd,eps,ncols,rlams,lenrlams,ipivots,w,lenw,
     2      lused)
        implicit real *8 (a-h,o-z)
        save
        dimension rlams(1),ipivots(1),
     1      par1(1),par2(1),par3(1),w(1),par4(1),par5(1)
c 
        external matfun
c 
c       this subroutine constructs the left singular vectors, the
c       singular values and the integer array of the most
c       "significant"
c       columns of the user-specified rectangular matrix
c 
...
c 
c    PLEASE NOTE THAT THIS SUBROUTINE IS ALMOST IDENTICAL TO THE
C    SUBROUTINE CMVECSVD; THE ONLY DIFFERENCE IS THE PRESENCE OF
C    THE INTEGER OUTPUT ARRAY IPIVOTS, CONTAINING THE SEQUENCE
C    NUMBERS OF THE "SIGNIFICANT" COLUMNS
c 
c                    Input parameters:
c 
...
c  ipivots - the integer array of length ncols, containing the sequence
c            of "significant" columns of a
c 
c                            Work array:
c 
c  w - must be big
...
c 
         call mvecpcini(nopt,nblocks,nlast)
c 
...
c 
        ...
        lenw2=lenw-lrhs-2
c 
        call mvecpcomp(jer,matfun,par1,par2,par3,par4,par5,n,
     1    nblocks,maxblock,eps,rlams,w(irhs),lenrlams,
     2      ncols,ipivots,w(iw),lenw2,lused)
c 
        call prinf('in mvecpsvd after mvecpcomp, ncols=*',ncols,1)
c 
        lused=lused+iw
c 
...
c 
        if(jer .ne. 0) then
            ier=16
            call prinf('bombing after mvecpcomp, with jer=*',jer,1)
            return
        endif
c 
        call mvecpmove(w(iw),w,ncols*n+2)
c 
        do 1500 i=1,ncols
c 
...
c 
        call mvecpsmallmat(w(ia),n,ncols,nvects,w(ib),par1,par2,par3,
     1      par4,par5,w(icol),w(icol2),matfun,eps)
  
  
cccc        call prin2('after mvecpsmallmat, b=*',w(ib),ncols**2)
c 
c       construct the SVD of the matrix b
c 
        call mvecpudv(w(ib),ncols,w(ia),eps,numrows,n)
  
        call prinf('after mvecpudv, numrows=*',numrows,1)
c 
...
c 
c 
        subroutine mvecpsmallmat(phis,n,ncols,m,b,par1,par2,par3,
...
c 
        do 1800 j=1,ncols
c 
        call mvecpscap(col,phis(1,j),n,prod)
c 
        b(j,i)=prod
 1800 continue
c 
...
c 
c 
        subroutine mvecpcrea(nummat,a,rhs,n,m,ipivots,matfun,
...
c 
 2000 continue
c 
cccc        call prin2('exiting mvecpcrea, a=*',a,n*m)
...
c 
c 
        entry mvecpcget(nummat,m7)
        imatr=nummat
        m7=nopt
        if(imatr .eq. nblocks) m7=nlast
        return
c 
c 
c 
        entry mvecpcini(nopt7,nblocks7,nlast7)
c 
        nopt=nopt7
        nblocks=nblocks7
        nlast=nlast7
...
c 
c 
        subroutine mvecpcomp(ier,matfun,par1,par2,par3,par4,
     1      par5,n,nblocks,maxblock,eps,rnorms,rhs,lenrhs,
     2      ncols,ipivots,w,lenw,lwused)
        implicit real *8 (a-h,o-z)
        save
...
c        by a prior call to the subroutine mvecpboxs (see)
...
c 
        call mvecpboxs(nblocks,w(iboxes),nboxes)
c 
...
c 
        ...
        lenw2=lenw-iw-1
c 
        call mvecprecu(ier,matfun,par1,par2,par3,par4,par5,
     1      maxblock,eps,w(iboxes),n,rnorms,rhs,lenrhs,n
     2      cols,ipivots,w(iw),lenw2,lwused)
c 
        call mvecpmove(w(iw),w,ncols*n+2)
c 
...
c 
c 
        subroutine mvecprecu(ier,matfun,par1,par2,par3,
     ...
     1      par3(1),par4(1),par5(1),   ipivots(1)
c 
        external matfun
c 
c       this subroutine compresses the user-provided matrix via a
...
c 
        ...
        lrnorms=mmax+2
c 
        call mvecpcget(nummat,m)
        iipivots=irnorms+lrnorms
        lipivots=m+10
c 
        ia=iipivots+lipivots
        la=mmax*n +10
c 
        ltot=ia+la
 ...
 1400 continue
c 
        call mvecpreac(matfun,par1,par2,par3,par4,par5,nummat,
     1    eps,w(ia),w(irhs),n,m,w(irnorms),ncols,w(iipivots))
c 
        index=boxes(1,ibox)
        call mvecpstor(ier,index,w(ia),w(irhs),n,ncols,w(irnorms),
     1      w(iipivots),w,lenw)
c 
        if(ier .ne. 0) return
c 
...
c 
        ...
        call mvecppnt(ier,index1,ia1,irhs1,n1,ncols1,irnorms1,
     1     iipivots1,w,inuse)
c 
        index2=boxes(1,ison2)
        call mvecppnt(ier,index2,ia2,irhs2,n,ncols2,irnorms2,
     1     iipivots2,w,inuse)
c 
        call prinf('before mvecpmrg, inuse=*',inuse,1)
c 
        lbuff=mmax*2+mmax*n+100
c 
        irhs=inuse +2 +lbuff
...
c 
 ...
 3000 continue
c 
        call mvecpmrg(w(ia1),ncols1,w(irnorms1),w(irhs1),w(iipivots1),
     1      w(ia2),ncols2,w(irnorms2),w(irhs2),w(iipivots2),
     2      n,eps,w(ia),ncols,w(irnorms),w(irhs),w(iipivots) )
c 
...
c 
        call mvecpmove(w(irnorms),rnorms,ncols+1)
        call mvecpmove(w(irhs),rhs,ncols+1)
        call mvecpmoveint(w(iipivots),ipivots,ncols+1)
        call mvecpmove(w(ia),w,ncols*n+2)
c 
        return
c 
 3600 continue
...
c 
c 
        subroutine mvecpstor(ier,index,a,rhs,n,ncols,rnorms,ipivots,
     1     w,lenw)
        implicit real *8 (a-h,o-z)
        save
        dimension a(1),rhs(1),rnorms(1),w(1),ipivots(1)
        integer *4 map(5,100)
c 
...
c 
        call mvecpmove(rnorms,w(irnorms),lrnorms)
        call mvecpmove(rhs,w(irhs),lrhs)
        call mvecpmove(ipivots,w(iipivots),lipivots)
        call mvecpmove(a,w(ia),la)
c 
...
c 
c 
        entry mvecpretr(ier,index,a,rhs,n,ncols,rnorms,ipivots,w)
c 
...
c 
        call mvecpmove(w(irnorms),rnorms,lrnorms)
        call mvecpmove(w(irhs),rhs,lrhs)
        call mvecpmove(w(iipivots),ipivots,lipivots)
        call mvecpmove(w(ia),a,la)
c 
        return
c 
c 
...
c 
c 
        subroutine mvecpmoveint(ix,iy,n)
        implicit real *8 (a-h,o-z)
        save
        dimension ix(1),iy(1)
c 
...
c 
c 
        subroutine mvecpreac(matfun,par1,par2,par3,par4,par5,
     1    nummat,eps,a,rhs,n,m,rnorms,ncols,ipivots)
        implicit real *8 (a-h,o-z)
        save
        dimension a(1),rhs(1),rnorms(1),par1(1),par2(1),par3(1),
     1    par4(1),par5(1),   ipivots(1)
c 
        external matfun
c 
c        construct the matrix number nummat
c 
        call mvecpcrea(nummat,a,rhs,n,m,ipivots,matfun,par1,par2,par3,
     1      par4,par5)
c 
...
c       right-hand side
c 
        call mvecppiv(a,n,m,rhs,rnorms,eps,ncols,ipivots)
c 
ccc        call prinf('after mvecppiv, m=*',m,1)
ccc        call prinf('after mvecppiv, ncols=*',ncols,1)
ccc        call prinf('after mvecppiv, ipivots=*',ipivots,ncols)
c 
        return
        end
c 
...
c 
c 
        subroutine mvecpboxs(n,boxes,nboxes)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        call mvecpbdiv(boxes(1,ibb),boxes(1,icurr),boxes(1,icurr+1),
     1      ifdiv,icurr)
c 
...
c 
c 
        subroutine mvecpbdiv(dad,son1,son2,ifdiv,icurr)
        implicit real *8 (a-h,o-z)
        save
...
c 
cccc        call prinf('in mvecpbdiv, dad=*',dad,10)
...
c 
c 
        subroutine mvecpmrg(a,ncolsa,rnormsa,rhsa,ipivotsa,
     1      b,ncolsb,rnormsb,rhsb,ipivotsb,n,eps,c,ncolsc,
     2    rnormsc,rhsc,ipivotsc)
        implicit real *8 (a-h,o-z)
        save
        dimension a(n,ncolsa),b(n,ncolsb),rnormsa(ncolsa),
     1      rnormsb(ncolsb),rnormsc(1),rhsa(ncolsa),rhsb(ncolsb),
     2      rhsc(1),c(n,1), ipivotsa(1),ipivotsb(1),ipivotsc(1)
...
cccc        call prinf('in mvecpmrg before mvecppiv, ncolsa=*',ncolsa,1)
cccc        call prinf('in mvecpmrg before mvecppiv, ncolsb=*',ncolsb,1)
cccc        call prinf('in mvecpmrg before mvecppiv, ncc=*',ncc,1)
...
cccc        call prin2('rnormsb=*',rnormsb,ncolsb)
        ...
        call mvecppiv(c,n,ncc,rhsc,rnormsc,eps,ncolsc,ipivotsc)
c 
ccc        call prinf('after mvecppiv, ncc=*',ncc,1)
ccc        call prinf('after mvecppiv, ncolsc=*',ncolsc,1)
ccc        call prinf('after mvecppiv, ipivotsc=*',ipivotsc,ncolsc)
c 
        return
        end
c 
...
c 
c 
        subroutine mvecpamatv(a,n,m,x,y)
        implicit real *8 (a-h,o-z)
        save
...
c 
c 
        subroutine mvecppiv(b,n,m,rhs,rnorms,eps,ncols,ipivots)
        implicit real *8 (a-h,o-z)
        save
        dimension b(n,m),rnorms(1),rhs(m),ipivots(1)
c 
...
c 
        ...
        do 2780 j=1,i-1
c 
        call mvecpscap(b(1,i),b(1,j),n,d)
c 
        do 2770 l=1,n
        b(l,i)=b(l,i)-b(l,j)*d
 2770 continue
...
c 
        call mvecpscap(b(1,i),b(1,i),n,d)
c 
...
c 
c 
        subroutine mvecpscap(x,y,n,prod)
        implicit real *8 (a-h,o-z)
        save
        dimension x(1),y(1)
c 
...
c 
c 
        subroutine mvecpudv(a,n,u,eps,numrows,nn)
        implicit real *8 (a-h,o-z)
        save
...
c 
 ...
 4200 continue
c 
       call mvecpsvsrt(a(1,1),n,u,v,a(1,2),ifuv,nn)
        return
        end
c 
c 
...
c 
c 
        subroutine mvecpsvsrt(s,n,u,v,iw,ifuv,nn)
        implicit real *8 (a-h,o-z)
        save
...
c 
c 
        subroutine mvecponeuv(a,rlam,u,v)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        vstar(2,2)=v(2,2)
c 
        call mvecpprod2(v,w,u)
c 
c       finally, compute the resulting diagonal elements
c 
        call mvecpprod2(u,a,z)
        call mvecpprod2(z,vstar,rlams)
          rlam(1)=rlams(1,1)
          rlam(2)=rlams(2,2)
c 
        u(1,2)=-u(1,2)

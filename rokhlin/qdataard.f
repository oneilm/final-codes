...
c 
c 
        subroutine qdataard(n,iw,name,arr)
        implicit real *16 (a-h,o-z)
        save
        dimension arr(1)
        character *3 name
...
c       statements are with 16 digits, to be compatible with
c       the usual double precision codes.
c 
c                     input parameters:
c 
...
c 
 1400 format(5x,a1,6x,e22.16,',',e22.16,',')
 1600 format(5x,a1,6x,e22.16,',',e22.16,'/')
c 
        do 1800 j=1,8
        if(j .ne. 8) write(iw,1400) ch1(j),
     1      arr((i-1)*16+j*2-1),arr((i-1)*16+j*2)
        if(j .eq. 8) write(iw,1600) ch1(j),
     1      arr((i-1)*16+j*2-1),arr((i-1)*16+j*2)
 1800 continue
 2000 continue
 3000 continue
...
c 
        ncards=ntail/2
        nntt=ntail-ncards*2
        if(nntt .ne. 0) ncards=ncards+1
c 
        do 3200 j=1,ncards
        if(j .ne. ncards) write(iw,1400) ch1(j),
     1      arr(ndata*16+j*2-1),arr(ndata*16+j*2)
        if( (j .eq. ncards) .and. (nntt .eq. 0) )
     1      write(iw,1600) ch1(j),
     2      arr(ndata*16+j*2-1),arr(ndata*16+j*2)
c 
 3100 format(5x,a1,6x,e22.16,'/')
c 
        if( (j .eq. ncards) .and. (nntt .eq. 1) )
     1      write(iw,3100) ch1(j),
     2      arr(ndata*16+j*2-1)
...
c 
c 
        subroutine arrequd(n,name,iw,nnt)
        implicit real *16 (a-h,o-z)
        save
        character *3 name
        character *1 ch1(20)
...
c 
        call prinf('in arrequd, n=*',n,1)
        call prinf('in arrequd, nnt=*',nnt,1)
 2200 format(9x, 'real *8')
c 
        ncards=n/4
        ntail=n-ncards*4
c 

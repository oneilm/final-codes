     ...
     2      whts3(1000),corners(2,500),w(1000 000)
        dimension rlens(1000)
c 
...
cccc        rk=ima*4
        ...
        iw=22
cccc        n=3000
cccc        call o1rauwrt(iw,n,wholemat)
  
  
cccc        iw=23
cccc        call o3rwrall(iw,w)
...
cccc        call boat36(corners,ncorners)
cccc        call boat8(corners,ncorners)
  
cccc        call gunboat2(corners,ncorners)
cccc        call gunboat(corners,ncorners)
  
        call prin2('corners as created*',corners,ncorners*2)
  
  
        do 1600 i=1,ncorners-1
        d1= (corners(1,i+1)-corners(1,i))**2
        d2= (corners(2,i+1)-corners(2,i))**2
        rlens(i)=sqrt(d1+d2)
 1600 continue
c 
        d1= (corners(1,1)-corners(1,ncorners))**2
        d2= (corners(2,1)-corners(2,ncorners))**2
        rlens(ncorners)=sqrt(d1+d2)
  
        call prin2('and lengths of sides are*',rlens,ncorners)
...
cccc        yreceiv=2
        ...
        call wholmatc(ir1,xsides,ysides,xsnorms,ysnorms,
     1      ncorners,whts3,wholemat,
     2      chnkpnt,interact2)
  
  
        call prin2('after wholmatc, wholemat=*',wholemat,160)
  
  
        call prinf('after wholmatc, ncorners=*',ncorners,1)
        npts=ncorners*20
     ...
     1      'polygon as constructed*')
        ...
        call prin2('after wholmatc, xsnorms=*',xsnorms,npts)
        call prin2('after wholmatc, ysnorms=*',ysnorms,npts)
  
  
        ir=22
  
        call dirsol2c(ir,xsides,ysides,xsnorms,ysnorms,
...
c 
c 
        subroutine dirsol2c(ir,xsides,ysides,xsnorms,ysnorms,
     1      n,whts3,wholematc,xsource,ysource,xreceiv,yreceiv,
     2      interact,rk)
        implicit real *8 (a-h,o-z)
        save
        dimension xsides(1),ysides(1),
     1     xsnorms(1),ysnorms(1),whts3(1),work(4000 000)
c 
        dimension uu(1000 000),vv(1000 000),ss(1000),
     1      ww(4000 000)
  
        complex *16 wholematc(n,n),rhs(1000),sol(1000),f1,f3,cd,
     1      h0,h1,ima,rk,f2
c 
...
c 
        ...
        do 2400 i=1,n
c 
        wholematc(i,i)=wholematc(i,i)-2*ima
 2400 continue
  
cccc        if(2 .ne. 3) goto 2500
  
        eps=1.0d-12
        lww=4000 000
        call csvdpiv(ier,wholematc,n,n,uu,vv,ss,ncols,eps,
     1      ww,lww,ltot)
  
        call prin2('after svdpivot, ss=*',ss,n*2)
...
c 
        call corthom(wholematc,n,work,cond)
...
c 
        cd=sqrt(d)*rk
        call hank101(cd,h0,h1)
  
        rhs(i)=h0
 1600 continue
  
        call prin2('rhs as created=*',rhs,n*2)
  
        call cmatvec8(wholematc,rhs,sol,n)
  
        call prin2('and sol=*',sol,n*2)
  
c 
c       evaluate the potential at the receiver via the
c       solution of the boundary value problem
c 
        call solevalc(xsides,ysides,xsnorms,ysnorms,
     1    whts3,sol,n,interact,xreceiv,yreceiv,f1)
c 
        call prin2('and f1=*',f1,2)
        ...
        call prin2('and f2=*',f2,2)
        call prin2('and f2-f1=*',f2-f1,2)
        call prin2('and f2/f1=*',f2/f1,2)
c 
c       evaluate the field via a grotesquely oversamppled
c       discretization of the boundary
c 
        call resaevac(ir,chnkpnt,interact,sol,n,
     1     xreceiv,yreceiv,f3)
  
  
        call prin2('and after resaevac, f3=*',f3,2)
        call prin2('and f3/f2-1=*',f3/f2-1,2)
  
        call prin2('and f3-f2=*',f3-f2,2)
...
c 
c 
        subroutine cmatvec8(amatr,x,y,nn)
        implicit real *8 (a-h,o-z)
        save
        complex *16 x(1),y(1),amatr(nn,nn),d
c 
        do 1400 i=1,nn
c 
        d=0
...
c 
c 
        subroutine cmatavec8(amatr,nn,nlarge,x,y)
        implicit complex *16 (a-h,o-z)
        save
        real *8 amatr(nn,nlarge)
        dimension x(1),y(1)
c 
        do 1400 i=1,nlarge
c 
...
c 
        complex *16 fout,ima,cd,h0,h1,rk,rk7
c 
c 
        data ima/(0.0d0,1.0d0)/
c 
...
cccc        call prin2('in interact2, rk=*',rk,2)
        call hank101(cd,h0,h1)
  
  
        fout=-h1*dd/sqrt(dx**2+dy**2)*rk -ima*rk*h0
  
cccc        fout=-h1*dd/sqrt(dx**2+dy**2) *rk
...
c       can be less that 5 degrees. The file contains 5 user-callable
c       subroutines: o2rrd2, o2rwrt2, o2rmatb2, o2rappb2, o2rvecne,
c       o2rsubm, o1rmtot, o3r1rece. Following is a description of these
c       subroutines:
c 
c 
c   wholmatc - constructs the matrix of interactions corresponding to
...
c 
c   o3r1rece - given a user-specified point in R^2 and definition of
c       a surface, o3r1rece constructs the linear form connecting the
c       values of the charge at the nodes of the discretization with
c       the value of the potential at the user-specified target
c 
c   o2rsubmc - constructs the matrix of interactions of a single 20-point
...
c 
c   o3rvecnec - returns to the user the coefficients of the linear form
...
c 
c   solevalc - determines the field generated at a point by a charge
c       (dipole, etc) distriburion on a surface.
c 
c   resaevac - resamples a user-specified function (conceptually, a
...
c 
c 
        subroutine wholmatc(ir,xsides,ysides,xsnorms,ysnorms,
...
c 
        dimension xs7(21),whts7(21),
     1      xys(21),whtsxys(21),xy(2),dxydt(2)
c 
        complex *16 totmat(20,20),wholemat(nsides*20,1)
c 
        external interact
c 
        data ifcalled/0/
...
c 
        ...
        ir11=ir
        call wholmat0c(ir11,nsides,chnkpnt,interact,wholemat)
c 
...
c 
        ...
        do 2200 i=1,nsides
c 
        call o1rmtotc(ir22,i,xs7,whts7,totmat,
...
c 
c 
        subroutine wholmat0c(ir,nchunks,chnkpnt,interact,wholemat)
        implicit real *8 (a-h,o-z)
        save
        complex *16 wholemat(nchunks*20,nchunks*20),amatr(20,20)
c 
        nc20=nchunks*20
        do 1400 i=1,nc20
        do 1200 j=1,nc20
...
c 
        if(i .eq. j) goto 2800
c 
        call o2rsubmc(irr,i,j,chnkpnt,interact,
     1      amatr)
c 
        irr=0
c 
...
c 
c 
        subroutine o2rsubmc(ir,ichunk,jchunk,chnkpnt,interact,
     1      amatr)
        implicit real *8 (a-h,o-z)
        save
        dimension targ(2),targnorm(2),
     1      dtardt(2),xs7(21),whts7(21),xys7(50),whtsxys7(100)
c 
        complex *16 quadr(24),amatr(20,24)
...
c 
        call o3rvecnec(ier,irr,jchunk,targ,targnorm,
...
c 
c 
        subroutine o3r1rece(ier,ir,targ,targnorm,
     1      vectquad,chnkpnt,interact,nsides)
        implicit real *8 (a-h,o-z)
        save
        external chnkpnt,interact
c 
        dimension targ(2),targnorm(2),whtsxys(22),
     1      xys(2,22),xs(22),whts(22)
c 
        complex *16 vectquad(1)
c 
c       Given user-specified oint in R^2 and definition of a surface,
c       this subroutine constructs the linear form connecting the
c       values of the charge at the nodes of the discretization with
c       the value of the potential at the user-specified target
c 
c 
...
c  ir - the FORTRAN file number from which the various precomputed data
c       are to be read. Hopefully, these data have been stored there by
c       a prior call to the subroutine o3rwrall (see). After the first call
c       to this subroutine (during a given run), this parameter can (and
c       should) be set to zero, to avoid rereading the data that have
c       already been read.
c 
c  targ - the target point in R^2
c  targnorm - the (unit) notmal at the target
...
c                   Output parameters:
c 
c  ier - tells the user whether the subroutine was able to evaluate
c        the potential at the point where it was requasted.
c      ier=0 means that the potential has been evaluated successfully
c      ier=4 means that the point was too close to one of the sides
c  vectquad - the coefficients of the linear form converting
c        the values of the charge" (dipole, etc.) distribution at the
c        support nodes into the value of the potential (normal
c        derivative of the potential, etc.) at the user-specified
c        point targ.
c 
c       . . . one chunk after another, construct the linear forms
c             connection the values of the charge at the nodes of the
c             discretization with the value of the potential at the
c             user-specified target
c 
        ier=0
        irr=ir
c 
        ...
        do 2000 ichunk=1,nsides
c 
        jj=(ichunk-1)*20+1
        call o3rvecnec(jer,irr,ichunk,targ,targnorm,
     1      xs,whts,vectquad(jj),chnkpnt,interact,xys,whtsxys)
c 
        irr=0
...
c 
c 
        subroutine o3rvecnec(ier,ir,ichunk,targ,targnorm,
     1      xs,whts,vectquad,chnkpnt,interact,xys,whtsxys)
        implicit real *8 (a-h,o-z)
        save
        real *8 xs(1),whts(1)
c 
        dimension xy(2),targ(2),targnorm(2),dxydt(2),whtsxys(1),
     1      thresh(2,26),dxydts(2,100),xynorm(2),xys(2,1)
c 
...
c 
        if(izone .eq. 1)
     1      call onevecquc(points2,weights2,coefslrg2,
     2          npts2,nregions2,ii,ifright,chnkpnt,interact,
     3          targ,targnorm,vectquad,ichunk)
c 
        if(izone .eq. 2)
     1      call onevecquc(points3,weights3,coefslrg3,
     2          npts3,nregions3,ii,ifright,chnkpnt,interact,
     3          targ,targnorm,vectquad,ichunk)
c 
        if(izone .eq. 3)
     1      call onevecquc(points4,weights4,coefslrg4,
     2          npts4,nregions4,ii,ifright,chnkpnt,interact,
     3          targ,targnorm,vectquad,ichunk)
c 
        if(izone .eq. 4)
     1      call onevecquc(points5,weights5,coefslrg5,
     2          npts5,nregions5,ii,ifright,chnkpnt,interact,
     3          targ,targnorm,vectquad,ichunk)
  
  
cccc        call prin2('after onevecquc, exiting from o3rvecnec, vectquad=*',
cccc     1      vectquad,40)
...
c 
c 
        subroutine onevecquc(points,weights,coefslrg,
...
c 
     ...
     1      points(npts,nregions),weights(npts,nregions),
     2      coefslrg(20,npts,nregions)
c 
        complex *16 fout,cd,formint(100),vectquad(1)
c 
        external interact,chnkpnt
c 
c 
...
c 
c 
        subroutine o1rmtotc(ir,ichunk,xs,whts,totmat,
     1      chnkpnt,interact,xys,whtsxys)
        implicit real *8 (a-h,o-z)
        save
        dimension numspts(21),weights(24,20),points(24,20),
     1      coefslrg(20,24,20),xys(2,1),whtsxys(1),xs(1),whts(1)
c 
        dimension xy(2),xynorm(2),targ(2),targnorm(2),dxydt(2),
     1      dtardt(2)
c 
        complex *16 rintmat(24,20),fout,cd,totmat(20,20)
c 
        external interact,chnkpnt
c 
c        This subroutine constructs the matrix of interactions
...
c 
c 
        subroutine solevalc(xsides,ysides,xsnorms,ysnorms,
     1    whts,sol,n,interact,xreceiv,yreceiv,fout)
        implicit real *8 (a-h,o-z)
        save
        dimension xsides(1),ysides(1),xsnorms(1),ysnorms(1),
     1      whts(1),xy(2),xynorm(2),targ(2)
c 
        complex *16 sol(1),fout,ff
c 
...
c 
c 
        subroutine resaevac(ir,chnkpnt,interact,sigma,npts,
     ...
     1      whtsxys(100 000)
c 
        complex *16 sigma(1),eta(100 000),f1,fout
...
c 
        ...
        jj=(i-1)*n+1
c 
        call cmatavec8(amatr,20,n,sigma(ii),eta(jj) )
 2600 continue
c 
...
c 
 ...
 1800 continue
c 
        call prin2('in resaevac, f1=*',f1,2)
c 
        return
        end

        ...
        complex *16 ima,z,h0,h1,h02,h12,zs(20 000),
     1      h0hank(20 000),h1hank(20 000),
     2      h0gaus(20 000),h1gaus(20 000)
        dimension zrea(2)
        data ima/(0.0d0,1.0d0)/
        equivalence (z,zrea(1))
c 
...
C 
         ...
         CALL PRIN2('z=*',z,2)
c 
         PRINT *, 'ENTER beamscle'
         READ *,beamscle
         CALL PRIN2('beamscle=*',beamscle,1)
c 
ccc          if(2 .ne. 3) goto 1200
         PRINT *, 'ENTER a'
         READ *,a
         CALL PRIN2('a=*',a,1)
c 
 ...
 1200 continue
c 
c        test the scaled hankel function computer
c 
cccc         beamscle=0
c 
         call gaubeam(z,h0,h1,beamscle)
         call prin2('after gaubeam, h0=*',h0,2)
         call prin2('after gaubeam, h1=*',h1,2)
c 
         call hank101(z,h02,h12)
c 
         h02=h02*dexp(-beamscle)
         h12=h12*dexp(-beamscle)
c 
         call prin2('after hank101, h02=*',h02,2)
         call prin2('after gaubeam, h12=*',h12,2)
c 
c       conduct massive testing
c 
ccc        a=70
ccc        ntest=30
        call gautest(ntest,zs,h0hank,h1hank,
     1      h0gaus,h1gaus,a,beamscle)
...
c 
c 
        subroutine gautest(n,z,h0hank,h1hank,
     1      h0gaus,h1gaus,a,beamscle)
        implicit real *8 (a-h,o-z)
        save
        complex *16 z(n,n),h0hank(n,n),h1hank(n,n),
     1      h0gaus(n,n),h1gaus(n,n),ima
        data ima/(0.0d0,1.0d0)/
c 
c        construct the test points
c 
        h=2*a/n+0.0001
c 
        do 1400 i=1,n
        do 1200 j=1,n
        z(j,i)=-a+(i-1)*h+ima*(-a+(j-1)*h)
 1200 continue
 1400 continue
         call prin2('array z as created*',z,n*n*2)
c 
c       evaluate the functions h0,h1 via both subroutines
c 
        errmax0=0
        errmax1=0
        dd=dexp(-beamscle)
        ...
        do 2200 j=1,n
c 
        call gaubeam(z(j,i),h0gaus(j,i),h1gaus(j,i),beamscle)
c 
        call hank101(z(j,i),h0hank(j,i),h1hank(j,i))
        h0hank(j,i)=h0hank(j,i)*dd
        h1hank(j,i)=h1hank(j,i)*dd
c 
        d0=cdabs( (h0hank(j,i)-h0gaus(j,i))/h0gaus(j,i))
        d1=cdabs( (h1hank(j,i)-h1gaus(j,i))/h1gaus(j,i))
        if(d0 .gt. errmax0) errmax0=d0
        if(d1 .gt. errmax1) errmax1=d1
 2200 continue
 2400 continue
         call prin2('array h0hank as created*',h0hank,n*n*2)
         call prin2('array h0gaus as created*',h0gaus,n*n*2)
        call prin2('errmax0=*',errmax0,1)
        call prin2('errmax1=*',errmax1,1)
        return
        end
c 
c 
...
c 
c 
        subroutine gaubeam(z,h0,h1,beamscle)
        implicit real *8 (a-h,o-z)
        save
        complex *16 z,h0,h1
c 
c        this subroutine evaluates the hankel functions H_0^1, H_1^1
c        for an arbitrary user-specified complex number z, scaled by
c        dexp(-beamscle).  its purpose in life is to work in a
c        regime where the subroutine hank101 will run into an
c        exponent overflow. in fact, this subroutine calls a slightly
c        modified version of hank101 (known here as gaub101). the
c        intended use of this subroutine is in the evaluation of
c        gaussian beams and their derivatives.
c 
c                      input parameters:
c 
c  z - the complex number for which the hankel functions
c        H_0, H_1 are to be evaluated
c  beamscle - the exponent of the scaling applied to the
c        hankel functions. more specifically, on exit
c        h0=H_0(z)*dexp(-beamscle),h1=H_1(z)*dexp(-beamscle).
c 
c                      output parameters:
c 
c  h0, h1 - the said scaled Hankel functions
c 
c 
c        . . . construct h0(z), h1(z), either scaled or not,
c              depending on the regime
c 
        call gaub101(z,h0,h1,beamscle,iffar)
cccc        call prinf('after gaub101, iffar=*',iffar,1)
c 
c        if the functions h0, h1 have not been scaled - scale them.
c 
        if(iffar .eq. 1) return
c 
        d=dexp(-beamscle)
        h0=h0*d
        h1=h1*d
...
c 
c 
        subroutine gaub101(z,h0,h1,beamscle,iffar)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        call gaub101u(z,ier,h0,h1,beamscle,iffar)
        return
 1400 continue
c 
...
c 
        ...
        call gaub101r(z,ier,h0,h1,beamscle,iffar)
        return
 2000 continue
c 
...
c 
        ...
        zr=-zu
c 
        call gaub101u(zu,ier,h0u,h1u,beamscle,iffar)
        call gaub101r(zr,ier,h0r,h1r,beamscle,iffar)
cccc         call prin2('in gaub101, h0u=*',h0u,2)
cccc         call prin2('in gaub101, h1u=*',h1u,2)
cccc         call prin2('in gaub101, h0r=*',h0r,2)
cccc         call prin2('in gaub101, h1r=*',h1r,2)
c 
...
c 
c 
        subroutine gaub101u(z,ier,h0,h1,beamscle,iffar)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        ccex=cdexp(ima*z)*cd
        m=35
        call gaub101p(c0p1,m,cd,h0)
cccc         call prin2('after gaub101p, h0=*',h0,2)
cccc        h0=h0*cdexp(ima*z)*cd
        h0=h0*ccex
        h0=h0*z**9
c 
        call gaub101p(c1p1,m,cd,h1)
cccc         call prin2('after gaub101p, h1=*',h1,2)
...
c 
        ...
        call gaub101l(z,h0,h1)
        return
c 
c        z is in the asymptotic regime. act accordingly.
c 
 4000 continue
        call gaub101a(z,h0,h1,beamscle,iffar)
        return
        end
c 
c 
...
c 
c 
        subroutine gaub101a(z,h0,h1,beamscle,iffar)
        implicit real *8 (a-h,o-z)
        save
        dimension p(18),q(18),p1(18),q1(18),rea(2)
        complex *16 z,zinv,pp,cd,qq,ima,h0,h1,pp1,qq1,
     1      com,z2,y0,y1,fj0,fj1,ser2,ser3,gausin,gaucos
        equivalence (rea(1),com)
        data ima/(0.0d0,1.0d0)/,pi/0.31415926535897932D+01/,
     1      done/1.0d0/,two/2.0d0/
c 
...
c 
        h0=pp+ima*qq
ccc        cdd=
cccc        h0=cdsqrt(2/pi/z)*cdexp(ima*(z-pi/4)) * h0
        h0=cdsqrt(2/pi/z)*cdexp(ima*(z-pi/4)-beamscle) * h0
  
        h1=pp1+ima*qq1
  
        h1=-cdsqrt(2/pi/z)*cdexp(ima*(z-pi/4)-beamscle) * h1*ima
c 
        return
c 
 2200 continue
...
c 
         call prin2('before gausinco, z2=*',z2,2)
        call gausinco(z2-pi/4,beamscle,gausin,gaucos)
c 
        fj0=( pp*gaucos-qq*gausin )*cdsqrt(2/pi/z2)
        y0= (pp*gausin+qq*gaucos ) *cdsqrt(2/pi/z2)
c 
        fj1=-( -pp1*gausin-qq1*gaucos )*cdsqrt(2/pi/z2)
        y1= -(pp1*gaucos-qq1*gausin)*cdsqrt(2/pi/z2)
  
  
  
cccc        fj0=( pp*cdcos(z2-pi/4)-qq*cdsin(z2-pi/4) )*cdsqrt(2/pi/z2)
cccc        y0= (pp*cdsin(z2-pi/4)+qq*cdcos(z2-pi/4) ) *cdsqrt(2/pi/z2)
c 
cccc        fj1=-( -pp1*cdsin(z2-pi/4)-qq1*cdcos(z2-pi/4) )*cdsqrt(2/pi/z2)
cccc        y1= -(pp1*cdcos(z2-pi/4)-qq1*cdsin(z2-pi/4))*cdsqrt(2/pi/z2)
c 
...
c 
c 
        subroutine gausinco(z,beamscle,gausin,gaucos)
        implicit real *8 (a-h,o-z)
        save
        complex *16 z,gausin,gaucos,ima,cdp,cdm
        data ima/(0.0d0,1.0d0)/,pi/0.31415926535897932D+01/,
     1      half/0.5d0/
c 
c       evaluate the sine and cosine of the argument z,
c       scaled by dexp(-beamscle)
c 
         call prin2('entered gausinco, z=*',z,2)
        cdp=cdexp(ima*z-beamscle)
        cdm=cdexp(-ima*z-beamscle)
        gausin=(cdp-cdm)*half
        gaucos=(cdp+cdm)*half
        return
        end
...
c        evaluate j0, j1
c 
cccc        call prin2('in gaub101l, z=*',z,2)
        m=16
        fj0=0
        fj1=0
        y0=0
...
c 
c 
        subroutine gaub101r(z,ier,h0,h1,beamscle,iffar)
        implicit real *8 (a-h,o-z)
        save
...
c 
        call gaub101p(c0p2,m,cd,h0)
cccc         call prin2('after gaub101p in second regime, h0=*',h0,2)
        h0=h0*cdexp(ima*z)/cdsqrt(z)
        m=31
        call gaub101p(c1p2,m,cd,h1)
cccc         call prin2('after gaub101p, h1=*',h1,2)

...
c 
	...
	h=1.0d-6
ccc        f=f* ( log(1-x) * (1-x))**2
ccc        f=f*  log(x*x+h*h)
        f=f*  log((x+0.99)*(x+0.99)+h*h)
c 
        return
        end
c 
...
c	approach.
c  maxmax - an integer indicates at what number of nodes we start to
c	using the L-function approach to find a good (or so we believe
c	so) starting nodes. This number usually is around 10. NOTE: if
c	one set this number large, say 1000, the effect will simply be
c	disabling the L-function approach.
c 
c                    Output parameters:
c 
...
c 
	...
	do 4000 ijk=nnnmax,1,-1
           call prinf('in homot0 downward pass, ijk=*',ijk,1)
c 
...
ccc	call prinf('in nodes2, numpts=*',numpts,1)
        ...
        do 1200 j=1,n
c 
        call nesteva(jer,w,ab,nn,k,xsout(i),j,f,der)
...
c 
	   xxx=xa+h*l
	do 3200 j=1,n
           call nesteva(jer,w,ab,nn,k,xxx,j,f,der)
	   if (jer.ne.0) goto 6008
           amatr(j,2*nnd-1)=f
           amatr(j,2*nnd)=der
 3200 continue
...
c 
c 
        subroutine matcpy(a,n,m,b)
        implicit real *8 (a-h,o-z)
c 
c	copy the matrix a(n,m) into b(n,m):
c 
        save
...
c 
	do j=1,n
	   ws(j)=rhs(j)
	enddo
***        call qrsol(bmatr,n,rhs,ws,rcond)
...
c 
	...
	time11=clotatim()
ccc	tottime=tottime+time11-time00
ccc	call prin2('in nodegau, tottime=*',tottime,1)
c 
	do j=1,2*n
	   sol(j)=rhs(j)
	enddo
***        call qrsol(amatr,n*2,rhs,sol,rcond)

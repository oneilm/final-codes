...
c 
        call mattest(a,n,c,u,v,uout,u2,errs,u4,prods,v4)
...
c 
c 
        subroutine mattest(a,n,c,u,v,uout,u2,errs,u4,prods,v4)
        implicit real *8 (a-h,o-z)
        save
...
c 
        a(i,i)=1
        if(i .lt. n) a(i+1,i)=i
cccc        if(i .lt. n) a(i+1,i)=1.0d-6
c 
cccc        a(i,i)=a(i,i)*(1+y(i))
cccc        if(i .lt. n) a(i+1,i)=a(i+1,i)*(1+y(i))
c 
 1400 continue
c 
...
c 
        ...
        call bidiag_svd(ier,aa,bb,n,eps,u4,v4,rlams,nlams,
     1      w,lenw,lused)
  
  
  
  
        call prin2('and v4=*',v4,n**2)
        call prin2('while u4=*',u4,n**2)
  
c 
c        check the orthogonality of v4
c 
  
        do 2400 i=1,n
        do 2200 j=1,n
c 
        call bidiagscap(v4(1,i),v4(1,j),n,prods(j,i))
 2200 continue
 2400 continue
c 
        call prin2('and prods=*',prods,n*n)
c 
c        multiply them things back, and compare with a
c 
        do 2800 i=1,n
        do 2600 j=1,n
c 
        prods(i,j)=u4(i,j)*rlams(j)
 2600 continue
 2800 continue
c 
  
        call matmulta(prods,v4,w,n)
  
        call prin2('and u rlams v=*',w,n*n)
...
c 
c 
c   bidiag_svd - constructs the singular values and the singular
...
c 
c 
        subroutine bidiag_svd(ier,aa,bb,n,eps,u,v,rlams,nlams,
     1      w,lenw,lused)
        implicit real *8 (a-h,o-z)
        save
        dimension aa(1),bb(1),w(1),u(n,n),rlams(1),v(n,1)
c 
c        This subroutine constructs the singular values and the
c        singular vectors of the user-supplied LOWER bidiagonal
...
c 
c        The input matrix is specified by the user in the form of
c        arrays aa, bb of diagonal and subdiagonal elements, respec-
c        tively. The output is in the form of the vector rlams of
c        the singular values, and the matrices u,v or orthonormal
c        vectors, such that (when the execution has been successful)
c 
c 
c                 A=U * Lambda * V^*,                               (0)
c 
c        where Lambda is the diagonal matrix with the elements of
c        the array rlams on the diagonal, and u, v are the output
c        matrices returned by the subroutine.
c 
c                      Input parameters:
c 
...
c  v - the (n,nlams) matrix of left singular vectors of A
...
c 
        call bidiag_svd0(jer,aa,bb,n,w(irhs),u,v,eps,
     1      w(iw2),lenw2,w(ia),w(ib),w(ia2),w(ib2),w(ic2),
     2      w(iuu),w(ivv),w(iww),w(iroots),w(inums_bad),
     3      w(iends_bad),rlams,lused2,nlams,prods)
c 
        lused=lused2+iw2
c 
        ier=jer
...
c 
c 
        subroutine bidiag_svd0(ier,aa,bb,n,rhs,u4,v4,eps,
     1      w,lenw,a,b,a2,b2,c2,uu,vv,ww,roots,nums_bad,
     2      ends_bad,rlams,lused,nlams,prods)
        implicit real *8 (a-h,o-z)
        save
        dimension aa(1),bb(1),a(1),b(1),roots(1),v4(n,n),
...
c 
        ...
        d2=1.0d20
cccc        if(i .ne. nroots) d2=abs(roots(i+1)-roots(i))
        d2=abs(roots(i+1)-roots(i))
c 
        d=d2
        if(d1 .lt. d2) d=d1
        delta=d/100
        if(d1 .gt. d2) delta=-d/100
        if(i .eq. nroots) delta=-abs(delta)
c 

...
c 
        ...
        eps=1.0d-15
c 
        call cadapgau(ier,a,b,fun,jtest,par2,m,eps,
     1      rint,maxrec,numint)
        call prin2('after cadapgau, rint=*',rint,2)
        call prinf('after cadapgau, ier=*',ier,1)
        call prinf('after cadapgau, maxrec=*',maxrec,1)
        call prinf('after cadapgau, numint=*',numint,1)
 2200 format(' and more accurately, rint= ',e22.16,2x,e22.16)
        write(6,2200) rint
        write(13,2200) rint
cccc          if(2 .ne. 3) stop
c 
...
ccc         fun=dlog(x)+dlog(1-x)+dlog(dabs(x-half))
cccc         fun=dlog(dabs(x))+dlog(dabs(done-x))
cccc          fun=half/x
ccc         call prin2('inside fun, fun=*',fun,1)
        fun=dcos(jtest*x)+ima*dsin(jtest*x)

...
c 
        lenw=1000 000
        call expocheb_prod(ier,a,b,x0,z,nmax,rhs,w,lenw,lused)
  
c 
        call prin2('after expocheb_prod0, rhs=*',rhs,nmax*2)
...
c        of the code for the evaluation of Chebychev-exponential
c        integrals
c 
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c 
c 
        subroutine expocheb_prod(ier,a,b,x0,z,nn,rhs,w,lenw,lused)
        implicit real *8 (a-h,o-z)
        save
        complex *16 z
        dimension w(1),rhs(1)
...
c 
c       \int_a^b T_n(x) \cdot e^{z \cdot (x-x0)} dx                     (1)
c 
c       for n=0,1,...,nn, where T_n denotes the n-th Chebychev
c       polynomial, and a,z,nn,x0 are user-supplied parameters. The
...
c 
        call expocheb_prod0(a,b,x0,z,nnn,w(irhs7),w(iaa),
...
c 
c 
        subroutine expocheb_prod0(a,b,x0,z,nn,rhs,
...
c 
c        construct the Chebychev polynomials of a and of 0
c 
        call chebpols(b,nn+5,pols)
c 
        call chebpols(a,nn+5,pols0)
c 
        do 2400 n=2,nn
c 
c 
...
c 
        ...
        tnm1=pols(n)
c 
        rhs(n)=( tnp1/(n+1)-tnm1/(n-1))/2 * rexb
c 
        tnp1=pols0(n+2)
        tnm1=pols0(n)
c 
        cd=( tnp1/(n+1)-tnm1/(n-1)) /2 * rexa
        rhs(n)=rhs(n)-cd
c 
        bb(n)=z/(2*(n+1))
        cc(n)=-z/(2*(n-1))
...
c 
        call expochheb_prod_fact(aa,bb,cc,nn,uu,vv,ww)
        call expocheb_prod_solve(uu,vv,ww,nn,rhs)
c 
c       shift them things
c 
        do 3600 i=nn,1,-1
...
c 
c 
        subroutine expochheb_prod_fact(a,b,c,n,u,v,w)
...
c 
c 
        entry expocheb_prod_solve(u,v,w,n,rhs)
c 
c        eliminate down
c 
        do 2400 i=1,n-1

...
cccc        call quaevol1(ier,ax,bx,k,funwht,funuser,
cccc     1      nfuns,epssvd,epsquadr,w,lw,lused,iw,ncols,nfintegr)
c 
...
c 
        ...
        if(ifout .eq. 2) ifout=3
        if(ifout .eq. 1) ifout=2
...
c 
        ...
        if(xs2(i) .gt. 1) xs2(i)=1 -1.0d-8
        if(xs2(i) .lt. -1) xs2(i)=-1 +1.0d-8
c 

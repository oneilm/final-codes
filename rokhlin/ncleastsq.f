...
c 
        do 1800 i=1,k
        do 1600 j=1,n
        c(j,i)=cdexp(-tt(i)*tt(j))+cdexp(-tt(i)/tt(j))
c 
cccc        if(i .eq. j) c(j,i)=c(j,i)+10
 ...
 1800 continue
c 
cccc        call prin2('c as created*',c,n*k*2)
...
c 
        ...
        t1=clotatim()
        call ncleamatrr(a,n,m,k,c,x,eps,ncols,
...
c 
cccc        call prin2('x as calculated*',x,n*k*2)
        call prin2('after ncleamatrr, errl2=*',errl2,1)
        call prin2('after ncleamatrr, errmax=*',errmax,1)
cccc        call prin2('after ncleamatrr, w=*',w,n*k*2)
...
c         ncleamatrl. Following is a brief description of these
c         subroutines.
c 

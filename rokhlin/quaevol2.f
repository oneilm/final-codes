...
c 
        ...
        iw=71
        nsings=3000
cccc        singmin=1.0d-5/16
cccc        singmax=1.0d-5/8
        ...
        singmin=1.0d-1
        singmax=1.0d-0
        call funuserini(singmin,singmax,nsings/3)
...
c 
        ...
        subroutine funuser7(x,i,f)
        implicit real *8 (a-h,o-z)
c 
        save
        dimension xs(10 000),ws(10 000),sings(10 000)
  
  
  
        if(i .lt. nsings) f=log(x**2+sings(i)**2)
        if(i .gt. nsings) f=sings(i-nsings)/(x**2+sings(i-nsings)**2)
        if(i .gt. 2*nsings) f=x**2/(x**2+sings(i-nsings*2)**2)
        if(i .eq. nsings) f=1
c 
        return
c 
...
c 
c 
        entry funuserini7(singmin7,singmax7,nsings7)
c 
        singmin=singmin7
        singmax=singmax7
        nsings=nsings7
        ...
        itype=1
        call legeexps(itype,nsings,xs,u,v,ws)
c 
        alpha=(singmax-singmin)/2
        beta=(singmax+singmin)/2
        do 1200 j=1,nsings
c 
        sings(j)=alpha*xs(j)+beta
 1200 continue
  
  
        call prin2('in funuserini, sings=*',sings,nsings)
...
c 
        ...
        f=1
        if(i .lt. nsings) f=log(x**2+sings(i)**2) *sqrt(ws(i))
  
  
        if( (i .gt. nsings) .and. (i .lt. nsings*2) )
     1      f=sings(i-nsings)/(x**2+sings(i-nsings)**2)
     2      *sqrt(ws(i-nsings))
  
        if(i .gt. 2*nsings) f=x**2/(x**2+sings(i-nsings*2)**2) *
     1      *sqrt(ws(i-nsings*2))
  
          if(i .gt. 2*nsings) f=(x**2-sings(i-nsings*2)**2)
     1      /(x**2+sings(i-nsings*2)**2)**2
     2      *sqrt(ws(i-nsings*2))
...
c 
c 
        entry funuserini(singmin7,singmax7,nsings7)
c 
        singmin=singmin7
        singmax=singmax7
        nsings=nsings7
...
c       stored
c  nnodes - the number of nodes in the quadrature to be returned. If this
c       parameter is .le. 0, it is ignored. IN MOST CASES, THIS PARAMETER
c       SHOULD BE SET TO (for example) -1. It only happens rarely that
c       the user knows how many nodes he wants in advance; usually, the
c       subroutine should be permitted to determine the required number
c       of nodes automatically.
c 
c                             Output parameters:
c 
...
c        than epsquadr (ralative to the first singular value); this is
...
c 
        ...
        iwhts=www(iwww+2)+iwww-1
c 
c       construct the quadratures
c 
        call quaevol0(ier,a,b,k,funwht,funuser,
     1      nfuns,epsquadr,www,lw,lused2,iw,www(irlams),
cccc     1      nfuns,epssvd,www,lw,lused2,iw,www(irlams),
     2      www(irints),www(iwww),ncols,www(ix),www(iwhts),nn,
     3      www(iamatr),nfintegr,
     4      www(iroots),www(iweights),www(irnorms),www(iipivots),
cccc     5      www(ixs2),www(iws2),www(iips),www(iwwww),lenwwww)
     5      www(ixs2),www(iws2),www(iwwww),lenwwww,nnodes)
c 
        return
        end
c 
...
c 
     ...
     1      nfuns,epssvd,epsquadr,www,lw,lused,iw,ncols,
     2      nfintegr,nnodes)
        implicit real *8 (a-h,o-z)
        save
        dimension www(1)
...
c 
     ...
     2      ncols,x,whts,nn,amatr,nfuns7,roots,weights,
     3      rnorms,ipivots,xs2,ws2,wwww,lwwww,nnodes)
c 
        implicit real *8 (a-h,o-z)
        save
        dimension coefs(1),x(1),amatr(1),www(1),rlams(1),
     1      rints(1),roots(1),weights(1),rnorms(1),ipivots(1),
     2      xs2(1),ws2(1),wwww(1),whts(1)
c 
...
c 
        ...
        nfuns7=i*2
c 
        if(nnodes .gt. 0) nfuns7=nnodes*2
        if(nfuns7 .gt. ncols) then
            ier=37
            return
        endif
c 
...
c 
        call quaeiniq(amatr,n,nfuns7,coefs,x,whts,
...
c 
 ...
 2600 continue
c 
 2800 format(10x,'Following is the ',i3,' - point quadrature ',
     1     /, 9x,' obtained via primitive pivoted Gram-Schmidt',
     2     /,10x)
        write(iw,2800) nfuns7
c 
...
c 
 ...
 3200 format(10x,'All of the following quadratures have been ',
     1    'obtained by ',/,10x,'consecutive elimination of nodes of ',
     2    'preceding quadratures')
        write(iw,3200)
c 
        call quaepnts(ier,nfuns7,rints,roots,weights,nfuns7,
     1      coefs,eps,xs2,ws2,wwww,lwwww,iw,a,b)
c 
        return
        end
c 
...
c 
     ...
     1      coefs,epsout,xs2,ws2,w,lw,iw,a,b)
        implicit real *8 (a-h,o-z)
        save
        dimension xs(1),ws(1),xs2(1),ws2(1),rints(1),w(1)
c 
...
c 
     ...
     1      coefs,epsout,w,lw,a,b,iflong)
c 
        if(jer .eq. 0) goto 1350
        imin=imin+1
c 
        if( (imin .gt. n ) .and. (iflong .eq. 1) ) then
...
c 
        if( (imin .gt. n ) .and. (iflong .eq. 0) ) then
c 
            imin=1
            iflong=1
            goto 2800
...
c       throwing out
c 
        call pntsignif(xs,ws,n,nfuns,xs2,coefs)
        call quabubb3(xs2,xs,ws,n-1)
c 
        call prin2('after quabubb3, signifs=*',xs2,n-1)
c 
        imin=1
c 
        jj=nfuns/2
...
c 
     ...
     2      w(isol),w(irhs),w(ixs22),w(iws22),a,b,ifout,iflong)
c 
...
c 
 ...
 2200 continue
c 
        if(ifout .ge. 4) goto 2600
        if(ifout .ge. 1) ifout=ifout+1
        if( (dnew .lt. epsout**2) .and. (ifout .eq. 0) ) ifout=1
c 
cccc        if( (i .eq. 30) .and. (dold/dnew .lt. 1.01) ) goto 2500
...
c 
     ...
     2      sol,rhs,xs2,ws2,a,b,ifout,iflong)
        implicit real *8 (a-h,o-z)
        save
...
c 
        if(ifout .ge. 1) then
c 
           call prinf('exiting on ifout=*',ifout,1)
c 
...
c 
        ...
        if(iflong .eq. 1) call prinf('iflong=1; kcontr=*',kcontr,1)
        if(iflong .eq. 1) goto 4000
...
c 
        ...
        call prin2('and dold=*',dold,1)
c 
cccc        if( (iflong .eq. 0) .and. (kcontr .gt. 4) ) ier=4
c 
        call mvecmove(xs2,xs,npts)
        call mvecmove(ws2,ws,npts)
c 
...
c 
c 
        subroutine quaeiniq(amatr,n,ncols,coefs,xs,whts,
     1      rints,roots,weights,rnorms,ipivots)
        implicit real *8 (a-h,o-z)
        save
        dimension amatr(ncols,n),coefs(1),xs(1),
     1      rnorms(1),roots(1),weights(1),
     2      ipivots(1),rints(1),whts(1)
c 
...
c 
        amatr(i,j)=val *sqrt(whts(j))
 1200 continue
 1400 continue
c 
c       conduct the pivoted Gram-Schmidt on amatr
c 
        call quaeppiv(amatr,ncols,n,rnorms,ipivots)
c 
...
c 
c 
        subroutine quaeppiv(b,n,m,rnorms,ipivots)
        implicit real *8 (a-h,o-z)
        save
...
c 
c 
        subroutine pntsignif(xs,ws,npts,nfuns,signifs,coefs)
        implicit real *8 (a-h,o-z)
        save
        dimension xs(1),ws(1),signifs(1)
c 
...
c 
        ...
        call nesteva(ier,coefs,xs(j),i,pol,der)
c 
        signifs(j)=abs(pol*ws(j)) **2+signifs(j)
 1400 continue
c 
        if(ws(j) .lt. 0) signifs(j)=ws(j)
 1600 continue
...
c 
c 
        subroutine quabubb3(signifs,xs,ws,npts)
        implicit real *8 (a-h,o-z)
        save
        dimension xs(1),ws(1),signifs(1)
c 

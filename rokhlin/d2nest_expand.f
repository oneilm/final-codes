...
c 
        ...
        call d2nest_expand(ier,xleft,xright,
     1      ybottom,ytop,n,eps,funeval2,par1,par2,iplot,
     2      w8,lenww,keep,lused)
...
c 
        call d2nest_eval(ier,zstest(1,iii),zstest(2,iii),w8,
     1      fout,derx,dery)
...
c 
        call d2nest_eval(ier,x,y,w8,
     1      fout,derx,dery)
c 
        call d2nest_eval(ier,x+h,y,w8,
     1      foutp,derxp,deryp)
c 
        call d2nest_eval(ier,x-h,y,w8,
     1      foutm,derxm,derym)
...
c 
        ...
        f=sin(z(1))+cos(z(2)) +log(z(1)**2+z(2)**2+0.01)
...
c        two-dimensional nested Chebychev expansions
c 
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c 
c 
c        This file contains two user-callable subroutines:
c        d2nest_expand, d2nest_eval. Following is a brief
c        description of these two subroutines.
c 
c   d2nest_expand - constructs a nested Chebychev expansion
...
c 
c   d2nest_eval - uses the data generated by a preceding
...
c 
c 
        subroutine d2nest_eval(ier,x,y,w8,fout,derx,dery)
        implicit real *8 (a-h,o-z)
        save
        dimension w8(1)
c 
...
c        call to the subroutine d2nest_expand to (approximately)
c        evaluate the user-supplied function at the user-supplied
c        point in R^2. Please note that this subroutine uses the
c        array w8 generated via a preceding call to d2nest_expand;
c        neither of these two subroutine has any known uses as a
...
c 
        ...
        iwork=w8(6)
c 
        call d2nest_eva0(x,y,w8(iboxes),w8(isides),ii,
...
c 
c 
        subroutine d2nest_eva0(x,y,boxes,sides,ii,
     1      ww,n,fout,derx,dery,w)
        implicit real *8 (a-h,o-z)
        save
        dimension sides(4,1),ww(1)
...
c 
        j=boxes(8,ii)
c 
        call ch2deva1(ww(j),n,xx,yy,fout,derx,dery,w)
c 
        derx=derx*ux
        dery=dery*uy
c 
...
c 
c 
        subroutine d2nest_expand(ier,xleft,xright,
...
c        of the user-supplied function funeval in the user-supplied
c        rectangle. The expansion is expected to be subesequently
c        used via the subroutine d2nest_eval (see). This subroutine
...
c 
        ...
        lenw7=lenw8-iw7
c 
        call d2nest_expan0(ier,xleft,xright,
...
c 
        ...
        lsides2=4*icurr+100
c 
        isides2=iboxes+lboxes2
        call d2nest_cpy(w8(isides),w8(isides2),lsides2)
        isides=isides2
c 
        iw72=isides+lsides2
        lw72=narrays*n**2+100
        call d2nest_cpy(w8(iw7),w8(iw72),lw72)
        iw7=iw72
c 
        w8(1)=iboxes+0.1
        w8(2)=isides+0.1
        w8(3)=iw7+0.1
        iw8keep=iw7+narrays*n**2+10
        w8(4)=iw8keep+0.1
        w8(5)=n+0.1
c 
        iwork=iw8keep+1
...
c 
        call d2nest_plot(iplot,w8(isides),icurr,w8(keep+10) )
c 
        return
        end
c 
...
c 
c 
        subroutine d2nest_expan0(ier,xleft,xright,
     1      ybottom,ytop,n,eps,funeval,par1,par2,
...
c 
        call d2nest_iset0(boxes(1,1),10)
c 
        boxes(1,1)=1
        boxes(2,1)=1
        boxes(3,1)=-7
...
c 
        narrays=0
        icurr=1
        call d2nest_iset0(boxes(1,icurr+1),10)
c 
        nzsplot=0
c 
        do 3000 lev=1,40
...
c 
        call d2nest_subdiv1(jer,boxes,sides,ibox,
...
c 
 4000 continue
c 
        call d2nest_cpy(w7(iww),w7,narrays*n**2+100)
c 
        return
        end
c 
...
c 
c 
        subroutine d2nest_subdiv1(ier,boxes,sides,ibox,
     1      w,n,funeval,par1,par2,eps,
     2      icurr,zs,coefs,fs,ww,narrays,lenww)
c 
        implicit real *8 (a-h,o-z)
        save
        dimension zs(2,1),w(1),sides(4,1),coefs(1),fs(1),
     1      ww(1)
        integer *4 boxes(10,1)
c 
...
c 
        ...
        ytop=sides(4,ibox)
c 
        call d2nest_ifdivide(xleft,xright,
...
c 
        call d2nest_cpy(coefs,ww(iarr),n**2)
        boxes(8,ibox)=iarr
        narrays=narrays+1
c 
        return
...
c 
        call d2nest_divide(xleft,xright,
     1      ybottom,ytop,
...
c 
c 
        subroutine d2nest_iset0(iarr,n)
        implicit real *8 (a-h,o-z)
        save
...
c 
c 
        subroutine d2nest_ifdivide(xleft,xright,
     1      ybottom,ytop,zs,n,w,funeval,par1,par2,eps,
     2      ifdivide,coefs,fs)
        implicit real *8 (a-h,o-z)
        save
        dimension zs(2,1),w(1),fs(1),coefs(n,n)
c 
c        construct the nodes discretizing this rectangle
c 
        call d2nest_discr(xleft,xright,
...
c 
c 
        subroutine d2nest_discr(xleft,xright,
...
c 
c 
        subroutine d2nest_divide(xleft,xright,
     1      ybottom,ytop,
     2      x1left,x1right,y1bottom,y1top,
     3      x2left,x2right,y2bottom,y2top,
     4      x3left,x3right,y3bottom,y3top,
...
c 
c 
        subroutine d2nest_plot(iw,sides,icurr,w)
        implicit real *8 (a-h,o-z)
        save

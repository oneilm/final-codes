        ...
        DIMENSION X(10000),W(10000),B(10000),NS(20),ENDPTS(2)
        CALL PRINI(6,13)
C 
C        CONSTRUCT ORDERS OF QUADRATURES
C 
        NUMQUA=8
        NS(1)=4
        DO 1200 I=3,NUMQUA,2
        NS(I)=NS(I-2)*2
 1200 CONTINUE
        DO 1400 I=2,NUMQUA,2
        NS(I)=(NS(I-1)+NS(I+1))/2
 1400 CONTINUE
CC      NUMQUA=7
        CALL PRINF('NS ARE*',NS,20)
C 
C       CONSTRUCT GAUSSIAN QUADRATURES AND STORE THEM ON DISK
C 
        KIND=0
        ALFA=0
        BETA=0
        ...
        DO 2000 I=1,NUMQUA
        N=NS(I)
        CALL PRINF('BEFORE GAUQ16,N=*',N,1)
        CALL GAUQ16(KIND,N,ALFA,BETA,KPTS,ENDPTS,B,X,W)
        CALL PRINF('AFTER GAUQ16,N=*',N,1)
 1600 FORMAT(2X,'ORDER  ',I6,'  NUMBER OF CARDS ',I6)
        WRITE(25,1600) N,N/2
 1800 FORMAT(2(1X,F19.17))
         WRITE(25,1800) (X(J),W(J),J=1,N)
         WRITE( 6,1800) (X(J),W(J),J=1,N)
         WRITE(13,1800) (X(J),W(J),J=1,N)
 2000 CONTINUE
         STOP
         END
C 
...
C 
C 
      SUBROUTINE GAUQ16(KIND, N, ALPHA, BETA, KPTS, ENDPTS, B, T, W)
C 
...
C 
C     INPUT PARAMETERS (ALL REAL NUMBERS ARE IN QUADRIPLE PRECISION)
C 
...
C 
C     OUTPUT PARAMETERS (BOTH QUADRIPLE PRECISION ARRAYS OF LENGTH N)
C 
...
C 
      implicit real *16 (a-h,o-z)
        save
      REAL*16 B(N), T(N), W(N), ENDPTS(2), MUZERO, T1,
     X GAM, SOLVE, QSQRT, ALPHA, BETA
C 
...
C 
      implicit real *16 (a-h,o-z)
        save
      REAL*16 SHIFT, A(N), B(N), ALPHA,SOLVE
C 
      ALPHA = A(1) - SHIFT
      NM1 = N - 1
      DO 10 I = 2, NM1
...
C 
      implicit real *16 (a-h,o-z)
        save
      REAL*16 A(N), B(N), MUZERO, ALPHA, BETA
      REAL*16 ABI, A2B2, QGAMMA, PI, QSQRT, AB
      DATA PI / 3.141592653589793238462643Q0/
C 
      done=1
      pi=qatan(done)*4
      NM1 = N - 1
...
C 
   10 MUZERO = 2.0Q0
      DO 11 I = 1, NM1
         A(I) = 0.0Q0
         ABI = I
   11    B(I) = ABI/QSQRT(4*ABI*ABI - 1.0Q0)
      A(N) = 0.0Q0
      RETURN
C 
...
C 
   50 AB = ALPHA + BETA
      ABI = 2.0Q0 + AB
      MUZERO = 2.0Q0 ** (AB + 1.0Q0) * QGAMMA(ALPHA + 1.0Q0) * QGAMMA(
     X BETA + 1.0Q0) / QGAMMA(ABI)
      A(1) = (BETA - ALPHA)/ABI
      B(1) = QSQRT(4.0Q0*(1.0Q0 + ALPHA)*(1.0Q0 + BETA)/((ABI + 1.0Q0)*
     1  ABI*ABI))
      A2B2 = BETA*BETA - ALPHA*ALPHA
      DO 51 I = 2, NM1
         ABI = 2.0Q0*I + AB
         A(I) = A2B2/((ABI - 2.0Q0)*ABI)
   51    B(I) = QSQRT (4.0Q0*I*(I + ALPHA)*(I + BETA)*(I + AB)/
     1   ((ABI*ABI - 1)*ABI*ABI))
      ABI = 2.0Q0*N + AB
      A(N) = A2B2/((ABI - 2.0Q0)*ABI)
      RETURN
C 
...
C              THAN -1.
C 
   60 MUZERO = QGAMMA(ALPHA + 1.0Q0)
      DO 61 I = 1, NM1
         A(I) = 2.0Q0*I - 1.0Q0 + ALPHA
   61    B(I) = QSQRT(I*(I + ALPHA))
      A(N) = 2.0Q0*N - 1 + ALPHA
      RETURN
      END
C 
...
C 
      ...
      REAL*16 D(N), E(N), Z(N), B, C, F, G, P, R, S, MACHEP
      REAL*16 QSQRT, QABS, QSIGN
C 
...
C     :::::::::: LOOK FOR SMALL SUB-DIAGONAL ELEMENT ::::::::::
  105    DO 110 M = L, N
            IF (M .EQ. N) GO TO 120
            IF (QABS(E(M)) .LE. MACHEP * (QABS(D(M)) + QABS(D(M+1))))
     X         GO TO 120
  110    CONTINUE
C 
  120    P = D(L)

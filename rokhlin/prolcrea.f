...
c 
        real *8 w(1000 000),khi(10 000),rlams(10 000),
     1      rlams2(10 000),fs(10 000),ders(10 000),
     2      fs2(10000),ders2(10000)
c 
        real *8 errs(10 000),errsder(10 000),
     1      fs3(10 000),errs3(10 000),rats(10000)
...
C 
        ...
        CALL PRINf('k=*',k,1 )
c 
c        construct the discretization of the interval [-1,1]
c 
        itype=1
        nn=1000
...
c 
c       use the obtained data to evaluate the prolate function at a
c       bunch of nodes
c 
c 
        do 2200 i=1,nn
c 
        call proleva(k,ts(i),w,fs(i),ders(i) )
c 
 2200 continue
c 
        iw=21
cccc        call lotagraph(iw,ts,fs,nn,'the prolate wave function*')

        call quagraph(iw,ts,fs,nn,3,'the prolate wave function*')

c 
c       evaluate the prolate function using the faster version of
c       proleva (known as proleval), and compare the results to those
c       obrained with proleva
c 
        do 2400 i=1,nn
c 
        call proleval(k,ts(i),w,fs2(i),ders2(i) )
  
        errs(i)=fs2(i)-fs(i)
        errsder(i)=ders2(i)-ders(i)
  
        rats(i)=fs2(i)/fs(i)
 2400 continue
c 
        call prin2('now, fs2=*',fs2,nn)
        call prin2('and fs=*',fs,nn)
  
  
        call prin2('and errs=*',errs,nn)
cccc        call prin2('and rats=*',rats,nn)
...
ccccc        stop
        ...
        call prin2('and errsder=*',errsder,nn)
  
  
c 
c       evaluate the prolate function using the faster version of
c       proleva that does not calculat derivatives (known as prolevv),
c       and compare the results to those obrained with proleva
c 
        do 2600 i=1,nn
c 
        call prolevv(k,ts(i),w,fs3(i) )
  
        errs3(i)=fs3(i)-fs(i)
 2600 continue
c 
        call prin2('and fs3 as calculated via prolevv are*',
     1      fs3,nn)
        call prin2('and errors in fs3 are*',errs3,nn)
 ...
 2800 continue
        ...
        call prin2('and rlams2(k)=*',rlams2(k),1)
        call prin2('and rlams2(k)-1=*',rlams2(k)-1,1)

        zero=0
        call prolevv(k,zero,w,fzero)

        call prin2('and fzero=*',fzero,1)

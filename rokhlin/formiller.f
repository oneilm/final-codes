...
c 
        ...
        save
        dimension tt(1000),
     1      fs(nn,1),ders(nn,1),ww(1000),coefs(10000),
     2      diffcoefs(10000),dervals(10000),der2vals(10000)
c 
...
c 
     ...
     1      errmax,1)
c 
c        differentiate the first prolate function and evaluate the
c        derivative of the differentiated expansion
c 
        i=0
        call prolunpk(i,w,coefs,ndiff)
        call prin2('coefficients from prolunpk are*',coefs,ndiff)
c 
        call legediff(coefs,ndiff,diffcoefs)
        call prin2('differentiated coefficients from prolunpk are*',
     1      diffcoefs,ndiff)
  
c 
c       evaluate the derivative and the second derivative
c       at the points on the interval [-1,1] and plot them
c 
        do 3200 i=1,nn
c 
cccc        call legeFDER(X,VAL2,der2,coefs,n)
  
        call legeFDER(tt(i),dervals(i),der2vals(i),diffcoefs,ndiff)
 3200 continue
  
        call prin2('values of derivative are*',dervals,nn)
        call prin2('values of second derivative are*',der2vals,nn)
...
c 
        ...
        iw=301
        call lotagraph(iw,tt,dervals,nn,
     1      'first derivative of psi_0*')
  
  
        iw=302
        call lotagraph(iw,tt,der2vals,nn,
     1      'second derivative of psi_0*')

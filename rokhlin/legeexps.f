        ...
        dimension pols(1000),u(10000),v(10000),
     1      ts(100 000),whts(100 000),w(100 000),fs(1000),
     2      pexp(1000),ainte(10000),endinter(1000),
     3      fs2(1000),fs3(1000),adiff(10 000),
     4      pjcoefs1(10000),pjcoefs2(10 000)
c 
        call prini(6,13)
C 
...
c 
         ...
         CALL PRINF('m=*',m,1 )
c 
c       test the new legendre polynomial routine
c 
  
        x=-1
        call legepols(x,n,pols)
        call prin2('pols=*',pols,n+4)
c 
c       test the legendre expansion routones
c 
         itype=2
cccc         itype=1
         call legeexps(itype,n,ts,u,v,whts)
         call prin2('after legeexps, ts=*',ts,n)
         call prin2('after legeexps, whts=*',whts,n)
...
cc         stop
         ...
         call prin2('after legeexps, u=*',u,n**2)
         call prin2('after legeexps, v=*',v,n**2)
c 
c 
        call matmul(u,v,w,n)
        call prin2('and u times v=*',w,n*n)
        call howbig(w,n)
c 
c       now, check how well this expansion works
c 
cccc        m=3
        do 2200 i=1,n
        fs(i)= fun(ts(i),m)
 2200 continue
        call prin2('fs =*',fs,n)
c 
        call matvec(u,fs,pexp,n)
        call prin2('pexp=*',pexp,n)
        ...
        call legeFDE2(Xtest,VAL,der,PEXP,N,
     1      pjcoefs1,pjcoefs2,ninit)
        call prin2('val from legefder=*',val,1)
        call prin2('val from fun=*',fun(xtest,m),1)
c 
c       now, check the derivative
c 
        call prin2('der=*',der,1)
  
        valana=fun(xtest,m)
        derana=m*xtest**(m-1)
c 
        call prin2('and analytically*',m*xtest**(m-1),1)
  
        ntest=100 000
        do 2400 i=1,ntest
  
        call legeFDER(Xtest,VAL,der,PEXP,N)
 2400 continue
...
c       new subroutine
c 
        call legeexe2(Xtest,VALnew,PEXP,N,
     1      pjcoefs1,pjcoefs2,ninit)
...
c 
c       evaluate both the function and the derivative vis
c       the newest subroutine
c 
        call legeFDER(Xtest,VAL3,der3,PEXP,N)
  
        call prin2('finally, val3=*',val3,1)
        call prin2('finally, der3=*',der3,1)
  
        call prin2('der3-derana=*',der3-derana,1)
        call prin2('and der3-der=*',der3-der,1)
        call prin2('and val3-valana=*',val3-valana,1)
c 
c       construct the matrix for the indefinite integration at
c       the Gaussian nodes and test it
c 
        itype=3
c 
cccc        call legeinmt(n,ainte,adiff,itype,w)
        call legeinmt(n,ainte,adiff,ts,whts,endinter,itype,w)
  
c 
        call prin2('after legeinm0, ainte=*',ainte,n*n)
        call prin2('after legeinm0, adiff=*',adiff,n*n)
c 
c       . . . check the integrator
c 
        dd=(-1.0d0)**(m+1)/(m+1)
        do 3200 i=1,n
        fs(i)=ts(i)**m
        fs3(i)=ts(i)**(m+1)/(m+1)-dd
 3200 continue
c 
        call matvec(ainte,fs,fs2,n)
c 
        call prin2('fs=*',fs,n)
        call prin2('after numerical integration, fs2=*',fs2,n)
        call prin2('and fs3=*',fs3,n)
c 
        do 3400 i=1,n
        fs3(i)=fs3(i)-fs2(i)
 3400 continue
        call prin2('and differences are*',fs3,n)
c 
c       now, differentiate the integrated f
c 
        call matvec(adiff,fs2,fs3,n)
c 
        call prin2('and after differentiation*',fs3,n)
c 
        do 3600 i=1,n
        fs3(i)=fs3(i)-fs(i)
 3600 continue
c 
        call prin2('and error in derivative*',fs3,n)
c 
        call prin2('and endinter=*',endinter,n)
c 
        valend=0
        do 3800 i=1,n
c 
        valend=valend+endinter(i)*fun(ts(i),m)
 3800 continue
        call prin2('valend=*',valend,1)
c 
        done=1
        valend2=fun(done,m)
        call prin2('and valend2=*',valend2,1)
        call prin2('and differenece is*',valend-valend2,1)
        stop
        end

...
cccc        real *8 ima,rk,a(100 000),b(100 000)
        real *8 ima,rk
c 
        complex *16 a(100 000),b(100 000)
...
cccc        call prin2('after matr_crea, a=*',a,n1*n2*2)
...
cccc        call prin2('after matr_crea, b=*',b,n2*n3*2)
...
c 
        lenw=1000 000
        call cskel_twomatr(ier,a,b,n1,n2,n3,eps,
...
c 
        ...
        dimension ipivots(1)
c 
        complex *16 a(n,m),b(m,k),askel(n,ncols),bskel(ncols,k),
     1      fillin(1),eval(1),expand(1)
c 
        complex *16 ab(1000 000),uu(1000 000),vv(1000 000),
     1      errs(1000000)
c 
...
c 
        call cskel_twomatr_matmult(a,b,ab,n,m,k)
c 
c       test the matrix fillin
c 
        call cskel_twomatr_matmult(fillin,bskel,uu,ncols,ncols,k)
        call cskel_twomatr_matmult(askel,uu,vv,n,ncols,k)
c 
...
c 
        call cskel_twomatr_matmult(eval,bskel,uu,m,ncols,k)
        call cskel_twomatr_matmult(a,uu,vv,n,m,k)
c 
c        calculate the difference between vv and ab
c 
        errmax=0
...
c 
        call cskel_twomatr_matmult(expand,b,uu,ncols,m,k)
        call cskel_twomatr_matmult(askel,uu,vv,n,ncols,k)
...
c        cskel_twomatr and cskel_twomatr_basic. Following is
c        a brief description of these subroutines.
c 
c 
c   cskel_twomatr - starts with constructing a two-sided
c        skeletonization of the complex matrix
c 
...
c        eps.
c 
c   cskel_twomatr_basic - constructs a two-sided skeletonization
...
c 
c 
        subroutine cskel_twomatr(ier,a,b,n,m,k,eps,
     1      ncols,ipivots,fillin,eval,expand,w,lenw,lused)
        implicit real *8 (a-h,o-z)
c 
        save
...
c 
        call cleamem(a,w(ia2),n*m)
        call cleamem(b,w(ib2),k*m)
c 
c        compress the matrix a*b
c 
        call cskel_twomatr_basic(w(ia2),w(ib2),n,m,k,eps,
...
c 
        ...
        call cskel_twomatr0(jer,a,b,n,m,k,ncols,ipivots,
...
c 
c 
        subroutine cskel_twomatr0(ier,a,b,n,m,k,ncols,ipivots,
...
c 
        delta=0
        call cleamatr(jer,askel,bskel,ab,n,ncols,ncols,k,eps,
...
c 
        call cleamatr(jer,a,bskel,ab,n,m,ncols,k,eps,
...
c 
        call cleamatr(jer,askel,b,ab,n,ncols,m,k,eps,
...
c 
c 
        subroutine cskel_twomatr_basic(a,b,n,m,k,eps,
     1      ipivots,ncols,rnorms)
        implicit real *8 (a-h,o-z)
        save
        dimension ipivots(1),rnorms(1)
        complex *16 a(n,m),b(m,k),cd
c 
c 
...
c 
        m0=1
        call cskel_twomatr_rnorms_calc(a,b,n,m,k,rnorms,m0,tot)
c 
        thresh=tot*eps**2
c 
...
c 
        ...
        do 2500 j=1,i-1
c 
        call cskel_twomatr_scap2(b,m,k,j,i,cd)
c 
        do 2400 l=1,k
...
c 
        ...
        call cskel_twomatr_scap2(b,m,k,i,i,cd)
        d=cd
c 
...
c 
        do 3200 j=i+1,m
c 
        call cleascap(a(1,i),a(1,j),n,cd)
c 
        do 3000 l=1,n
 ...
 3200 continue
c 
c        orthogonalize all subsequent rows of b to
c        the row number i
c 
        do 3600 j=i+1,m
c 
...
c 
        call cskel_twomatr_scap2(b,m,k,i,j,cd)
c 
        do 3400 l=1,k
...
c 
        m0=i+1
        if(m0 .ne. m+1)
     1      call cskel_twomatr_rnorms_calc
     2      (a,b,n,m,k,rnorms,m0,tot)
c 
...
c 
c 
        subroutine cskel_twomatr_matmult(a,x,c,n,m,k)
...
c 
c 
        subroutine cskel_twomatr_rnorms_calc
     1      (a,b,n,m,k,rnorms,m0,tot)
        implicit real *8 (a-h,o-z)
        save
        dimension rnorms(1)
        complex *16 a(n,m),b(m,k)
c 
c       . . . prepare the arrays of values of norms
c             of columns of a and rows of b
c 
...
c 
c 
        subroutine cskel_twomatr_scap2(b,m,k,ii,jj,prod)
        implicit complex *16 (a-h,o-z)
        save
        dimension b(m,k)
c 

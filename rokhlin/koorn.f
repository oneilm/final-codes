...
cc      n=1
      t1=second()
      do i=1,n
      call koorn2eva(mmax,z,pols2,w)
      enddo
      t2=second()
      call prin2('pols=*', pols2, (mmax+1)*(mmax+2)/2)
      call prin2('and speed=*', n/(t2-t1), 1)

c      goto 5000
c      stop


       call koorn2eva3(mmax,z,pols2,dersx2,dersy2,w)
...
c
      z1(1)=z(1)-h
      z1(2)=z(2)
      call koorn2eva3(mmax,z1,pols1,dersx2,dersy2,w)
ccc      call prin2('pols=*', pols1, (mmax+1)*(mmax+2)/2)
      z1(1)=z(1)
      z1(2)=z(2)
      call koorn2eva3(mmax,z1,pols2,dersx2,dersy2,w)
ccc      call prin2('pols=*', pols2, (mmax+1)*(mmax+2)/2)
      z1(1)=z(1)+h
      z1(2)=z(2)
      call koorn2eva3(mmax,z1,pols3,dersx2,dersy2,w)
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      do i=1,kk
         call koorn2eva(mmax,zs(1,i),pols1(1,i),w)
      enddo
      do i=1,kk
         do j=1,nvals
            pols2(i,j) = pols1(j,i)
...
c
c
      subroutine koorn2eva(mmax,z,pols,w)
c
...
c     increasing order. 
c
c     This subroutine is based on the Koornwinder representation
...
c
      ...
      iw2=iw1+mmax+1
      iw3=iw2+(mmax+1)**2
      call koorn2eva0(mmax,z,pols,w(iw1),w(iw2))
      return
      end
c
c
...
c
c
      subroutine koorn2eva0(mmax,z,pols,f1,f2)
c
...
c
c      x=-1.0d0/3.0d0-1.0d0/sqrt3*b+a
c      y=1.0d0/9.0d0*(-sqrt3+6*b)*sqrt3
      x=r11+r12*b+a
      y=r21+r22*b
c
...
c
      par1=(2*x+1+y)/2
      par2=(1-y)/2
      call koornlegepolsy(par1,par2,mmax,f1)
      do m=0,mmax
         par1=2*m+1
ccc         call koornjacopols(y,par1,zero,mmax,f2(1+m*(mmax+1)))
         call koornjacopols(y,par1,zero,(mmax-m),f2(1+m*(mmax+1)))
      enddo
...
c
c
      subroutine koorn2eva3(mmax,z,pols,dersx,dersy,w)
c
c     This subroutine evaluates at the user-supplied point z
c     a collection of polynomials (of x,y) orthogonal on the
...
c
      ...
      iw6=iw5+(mmax+1)**2
      iw7=iw6+(mmax+1)**2
      call koorn2eva30(mmax,z,pols,dersx,dersy,
     $     w(iw1),w(iw2),w(iw3),w(iw4),w(iw5),w(iw6))
      return
      end


      subroutine koorn2eva30(mmax,z,pols,dersx,dersy,
     $     f1,f2,f3,f4,f5,f6)
c
c     evaluate the orthonormal polynomials on the triangle, 
...
c
      ...
      u=(2*x+1+y)/(1-y)
      v=y
      call koornlegepols2(u,mmax,f3,f4)
      do m=0,mmax
         par1=2*m+1
         call koornjacopols2(v,par1,zero,mmax-m,
...
c
c
        subroutine koornjacopol1(x,a,b,n,pol)
        implicit real *8 (a-h,o-z)
        save
c
        pkm1=1
...
c
        ...
        subroutine koornjacopols(x,a,b,n,pols)
        implicit real *8 (a-h,o-z)
        save
        dimension pols(1)
c
...
c
c
        subroutine koornjacopol(x,a,b,n,pol,der)
        implicit real *8 (a-h,o-z)
        save
c
        pkm1=1
...
c
c
        subroutine koornjacopols2(x,a,b,n,pols,ders)
        implicit real *8 (a-h,o-z)
        save
...
c
c
        subroutine koornlegepolsy(x,y,n,pols)
        implicit real *8 (a-h,o-z)
        save
c
c       calculate a sequence of scaled Legendre polynomials 
...
c
c
        subroutine koornlegepolsy1(x,y,n,pol)
        implicit real *8 (a-h,o-z)
        save
c
c       calculate a sequence of scaled Legendre polynomials 
...
c
        ...
        subroutine koornlegepol1(x,n,pol,der)
        implicit real *8 (a-h,o-z)
c
        pkm1=1
        pk=x
...
c
        pol=pkp1
        der=dkp1
ccc        der=n*(x*pkp1-pk)/(x**2-1)
        return
        end



        subroutine koornlegepols2(x,n,pol,der)
        implicit real *8 (a-h,o-z)
        dimension pol(1),der(1)
c
        pkm1=1
        pk=x
        dkm1=0

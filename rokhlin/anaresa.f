        ...
        dimension t(15000),x(15000),y(15000),rls(15000),
     1      xgauss(1000),tgauss(1000),tsout(1000),xsout(1000),
     2      ysout(1000)
...
c 
        ...
        rl=pi*2
        eps=1.0d-12
        call anaresa(ier,funcurv2,rl,n,eps,t,h,rltot)
cccc        call anaresa(ier,funcurv3,rl,n,eps,t,h)
        call prin2('after anaresa, rl=*',rl,1)
        call prin2('after anaresa, t=*',t,n+1)
        call prinf('after anaresa, ier=*',ier,1)
        call prin2('after anaresa, h=*',h,1)
...
c 
        rls(i)=(i-1)*h
c 
cccc        call funcurv3(t(i),x(i),y(i),dxdt,dydt)
       call funcurv2(t(i),x(i),y(i),dxdt,dydt)
 1400 continue
c 
        iw=24
        call lotaplot(iw,x,y,n,'equispaced nodes*')
  
  
        call prin2('and rls=*',rls,n+1)
c 
...
c 
        ...
        xtest=0.7
c 
        call anapoint(ier,xtest,n,h,t,funcurv2,eps,
     1       tout,xout,yout,dxdtout,dydtout)
  
  
        call prinf('after anapoint, ier=*',ier,1)
cccc        stop
...
c 
        ...
        call prin2('tgauss as created*',tgauss,ngauss)
...
c 
        ...
        beta=rltot/2
        do 2200 i=1,ngauss
        xgauss(i)=alpha*tgauss(i)+beta
 2200 continue
  
        call prin2('xgauss as created*',xgauss,ngauss)
        call prin2('while rltot=*',rltot,1)
c 
...
c 
        ...
        call anapoint(ier,xgauss(i),n,h,t,funcurv2,eps,
     1       tsout(i),xsout(i),ysout(i),dxdtout,dydtout)
...
cccc        call prini(6,13)
        ...
        iw=25
        call lotaplot(iw,xsout,ysout,ngauss,'Gaussian nodes*')
...
c 
c 
        subroutine funcurv2(t,x,y,dxdt,dydt)
        implicit real *8 (a-h,o-z)
cccc          call prin2('in funcurve, t=*',t,1)
        save
        x=dcos(t)*10
        y=dsin(t)
        dxdt=-dsin(t)*10
...
c 
c 
        subroutine funcurv3(t,x,y,dxdt,dydt)
        implicit real *8 (a-h,o-z)
cccc          call prin2('in funcurve, t=*',t,1)
c 
        save
...
c        start of the actual resampling routines.
c 
...
c 
c 
        subroutine anaresa(ier,funcurve,rl,n,eps,t,h,rltot)
        implicit real *8 (a-h,o-z)
        save
        dimension t(1)
        external funcurve
...
c 
        call funcurve(t(k),x,y,dxdt,dydt)
c 
...
c       newton step
c 
        call funcurve(t(k)+dt,x,y,dxdt,dydt)
        dldt=dsqrt(dxdt**2+dydt**2)
c 
        derr=htrue-h
...
c 
        ...
        dt=(dt+dtold)/2
          call prin2('step-length control activated, dt=*',dt,1)
        ker=1
 1200 continue
 1300 continue
c 
...
c 
 1400 continue
        call prin2('newton failed, derr=*',derr,1)
        ier=1
 2000 continue
        t(k+1)=t(k)+dt
 3000 continue
...
c       if(jer .ne. 0) jer=1
        ...
        call prinf('exiting anaresa, ier=*',ier,1)
        return
        end
c 
c 
...
c 
c 
         subroutine anapoint(ier,x,n,h,ts,funcurve,eps,
     1       tout,xout,yout,dxdtout,dydtout)
         implicit real *8 (a-h,o-z)
        save
         real *8 ts(1)
...
c       newton step
c 
        call funcurve(tk,xk,yk,dxdt,dydt)
        dldt=dsqrt(dxdt**2+dydt**2)
c 
        tkold=tk
        tk=tk-fk/dldt *step
...
c 
        tout=tk
        call funcurve(tout,xout,yout,dxdtout,dydtout)
c 

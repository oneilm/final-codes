...
c 
        complex *16 val,der,val2,vals(10 000),
...
c 
        call cnestex_cheb(ier,a,b,k,fun1,
...
c 
        xtest=0.7
        call cneste_cheb(ier,w2,xtest,val,der)
c 
        ...
        do 2400 i=1,ntest
c 
        call cneste2_cheb(ier,w2,xstest(i),vals(i))
c 
...
c 
        ...
        call cneste_cheb(ier,w2,xtest,val,der)
        call cneste_cheb(ier,w2,xtest+h,valp,derp)
        call cneste_cheb(ier,w2,xtest-h,valm,derm)
c 
        der2=(valp-valm)/h/2
...
c       evaluation of nested Chebychev expansions of complex functions
c       of one real variable. There are 3 user-callable subroutines in
c       this file: cnestex_cheb, cneste_cheb, cneste2_cheb (actually,
c       cneste2_cheb is an entry in the subroutine cneste_cheb. Following
c       is a brief description of these subroutines.
c 
c   cnestex_cheb - finds a subdivision of the interval [a,b], such that on
...
c       subroutines cneste_cheb, cneste2_cheb (see) to evaluate the
c       interpolated function at arbitrary points on the interval [a,b].
c 
c   cneste_cheb - evaluates at the point x \in [a,b] the complex
...
c        subroutine cnestex_cheb (see) as its input; it has no known uses
c        as a stand-alone device
c 
c   cneste2_cheb - evaluates at the point x \in [a,b] the complex function
...
c        subroutine uses output of the subroutine cnestex_cheb (see) as its
...
c 
c 
        subroutine cnestex_cheb(ier,a,b,k,fun,par1,par2,par3,
...
c       output of this subroutine can be used by the subroutines cneste,
c       cneste2_cheb (see) to evaluate the function fun at arbitrary points
...
c     subroutines cneste, cneste2_cheb (see) for the evaluation
...
c     and the subsequent calls to the subroutines cneste, cneste2_cheb
c     (see)
c 
...
c 
        ...
        lenab=lenw2-iab
c 
         call callinco_cheb(ier,a,b,k,fun,par1,par2,par3,eps,
...
c 
        call cnodewht_cheb(w2(iab),nn,k,w2(ix))
c 
...
c 
        do 1400 i=1,n
c 
        call fun(w2(ix+i-1),par1,par2,par3,w2(ifs+2*i-1) )
 1400 continue
c 
        do 2200 i=1,nn
c 
        ii=(i-1)*k+1
        call calqrma2_cheb(w(iu),w2(ifs+2*ii-1),w(iww),k,1)
c 
        do 1800 j=1,k*2
c 
        w2(ifs+2*ii+j-3)=w(iww+j-1)
 1800 continue
c 
 2200 continue
c 
...
c 
c 
        subroutine cneste_cheb(ier,w2,x,val,der)
        implicit real *8 (a-h,o-z)
        save
        complex *16 val,der
        dimension w2(1)
...
c        complex function (together with its derivative) specified
...
c        of the subroutine cnestex_cheb (see) as its input; it has
...
c                           constructed (see subroutine cnestex_cheb)
...
c 
        call cneste_cheb0(ier,w2(icoefs),w2(iab),nn,k,x,val,der)
c 
        return
c 
c 
c 
c 
        entry cneste2_cheb(ier,w2,x,val)
c 
c        This entry evaluates at the point x \in [a,b] the complex
...
c        subroutine uses output of the subroutine cnestex_cheb (see)
...
c       evaluate the function and its derivative
c 
        call cneste2_cheb0(ier,w2(icoefs),w2(iab),nn,k,x,val)
c 
        return
        end
c 
...
c 
c 
        subroutine cneste_cheb0(ier,coefs,ab,nn,k,x,val,der)
        implicit real *8 (a-h,o-z)
        save
        dimension ab(2,1)
c 
...
c 
        ier=0
        call findinte_cheb(ier,x,ab,nn,intnum)
        if(ier .ne. 0) return
c 
...
c 
        jj=(intnum-1)*k+1
c 
        call chfuncder(t,VAL,der,coefs(jj),k-1)
c 
        der=der*u
c 
        return
...
c 
c 
        subroutine cneste2_cheb0(ier,coefs,ab,nn,k,x,val)
        implicit real *8 (a-h,o-z)
        save
        dimension ab(2,1)
c 
...
c        note that this subroutine uses output of the subroutine cnestex_cheb
...
c 
        jj=(intnum-1)*k+1
c 
        call chebcval(t,VAL,coefs(jj),k-1)
c 
        return
        end
c 
...
c 
c 
        subroutine cnodewht_cheb(ab,nn,k,x)
        implicit real *8 (a-h,o-z)
        save
...
c 
c 
         subroutine callinco_cheb(ier,a,b,k,fun,ipar1,par1,
...
c       all the work is done by the routine callinte_cheb (see).
c 
c                          input parameters:
c 
c  a,b - the ends of the interval to be subdivided
...
c 
c       . . . allocate memory for the subroutine callinte_cheb
c 
        ier=0
c 
        it=1
...
c 
         call callinte_cheb(ier,a,b,fun,ipar1,par1,par2,k,ab,lab,
...
c 
c 
         subroutine callinte_cheb(ier,a,b,fun,ipar1,par1,par2,
...
c 
        d=ab(2,nn)-ab(1,nn)
c 
        call cifsplit_cheb(ier,ab(1,nn),ab(2,nn),k,fun,ipar1,par1,
...
c 
c 
        subroutine cifsplit_cheb(ier,a,b,k,fun,ipar1,par1,
...
c       the interval
c 
        call calqrma2_cheb(u,f,coefs,k*2,k+1)
c 
...
c 
c 
        subroutine calqrma2_cheb(a,x,y,n,n0)
        implicit real *8 (a-h,o-z)
        save
        dimension a(n,n)
...
c 
c 
        subroutine findinte_cheb(ier,x,ab,nn,intnum)
        implicit real *8 (a-h,o-z)
        save
        dimension ab(2,nn)
c 

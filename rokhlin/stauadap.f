...
c 
c        test stauexp for a linear system of equations
c 
cccc        call test1(w,phiout,errs)
c 
c        test stauadap for a linear system of equations
c 
cccc        call test2(w,phiout,errs)
c 
c        test stauadap for a non-linear system of equations
c 
cccc         call test3(w,phiout,errs)
  
c 
c        test stauexp for a non-linear system of equations
c 
...
c 
c        test stauexp for a non-linear system of equations
c        that is not an autinomous system
c 
...
c 
c        test stauadap for a Jacobi elliptic functions sn, cn, cn
c 
...
c 
     ...
     1      tout(10 000),aa(1000),bb(1000),errs(1)
        complex *16 ima,clams(1000),
     1      phiout(1),phi00(1000),phi11(1000)
...
c 
        ...
        par1=rm
        call stauadap(ier,n,ncorrect,aaa,bbb,phi00,funeva4,
     1    funder4,m,par1,m,clams,eps,stepinit,maxinte,ifsave,
     2    aa,bb,ninter,tout,phiout,phi11,nfcalls,nfinv,ifinit,w,
     3    lw,ltot)
c 
cccc        tt2=second()
cccc        call prin2('time for stauadap is*',tt2-tt1,1)
        call prinf('after first stauadap, ier=*',ier,1)
        call prinf('after first stauadap, nfcalls=*',nfcalls,1)
        call prinf('after first stauadap, nfinv=*',nfinv,1)
        call prinf('after first stauadap, ltot =*',ltot,1)
...
c 
        call errprjac(tout,phiout,clams,ninter,n,m,errs,phi11,
...
c 
        ...
        implicit real *8 (a-h,o-z)
        save
        complex *16 phi(m),f(m)
  
  
cccc        call prin2('in funeva4, rm=*',rm,1)
...
c 
 1700 continue
        call prin2('and values at the right end =*',phi11,m*2)
c 
...
c 
        ...
        dimension w(1),errs(10 000),
     1      tout(10 000),aa(1000),bb(1000),errs(1)
        complex *16 clam,ima,clams(1000),
     1      phiout(1),phi00(1000),clam22,phi11(1000)
...
c 
        ...
        par1=apar
        call stauadap(ier,n,ncorrect,aaa,bbb,phi00,funeva3,
...
c       calculate and print the error
c 
        call errprnon(tout,phiout,clams,ninter,n,m,errs,phi11,
...
c 
cccc        tt1=second()
        call stauexp(ier,n,n2,ncorr,ncorr2,aaa,bbb,phi00,
     1    funeva3,funder3,m,par1,m,clams,eps,stepinit,maxinte,ifsave,
     2    aa,bb,ninter,tout,phiout,phi11,nfcalls,nfinv,ifinit,w,
     3    lw,ltot)
c 
cccc        tt2=second()
cccc        call prin2('time for stauexp is*',tt2-tt1,1)
        call prinf('after first stauexp, ier=*',ier,1)
        call prinf('after first stauexp, nfcalls=*',nfcalls,1)
        call prinf('after first stauexp, nfinv=*',nfinv,1)
        call prinf('after first stauexp, ltot =*',ltot,1)
        call prinf('after first stauexp, ninter =*',ninter,1)
...
cccc        tt1=second()
        call stauexp(ier,n,n2,ncorr,ncorr2,aaa,bbb,phi00,
     1    funeva2,funder2,m,par1,m,clams,eps,stepinit,maxinte,ifsave,
     2    aa,bb,ninter,tout,phiout,phi11,nfcalls,nfinv,ifinit,w,
     3    lw,ltot)
c 
cccc        tt2=second()
...
c 
        ...
        par1=apar
        call stauadap(ier,n,ncorrect,aaa,bbb,phi00,funeva2,
     1    funder2,m,par1,m,clams,eps,stepinit,maxinte,ifsave,
     2    aa,bb,ninter,tout,phiout,phi11,nfcalls,nfinv,ifinit,w,
     3    lw,ltot)
c 
cccc        tt2=second()
...
c 
        ...
        subroutine errprnon(tout,phiout,clams,ninter,n,m,errs,
...
c 
        ...
        implicit real *8 (a-h,o-z)
        save
        complex *16 phi(m),a(m,m),dermat(m,m)
c 
c       evalate the differential of the rhs of the differential equation
c 
...
cccc        tt1=second()
c 
        call stauadap(ier,n,ncorrect,aaa,bbb,phi00,funeva5,
...
c 
        ...
        implicit real *8 (a-h,o-z)
        save
        complex *16 phi(m),dermat(m,m),clams(1)
c 
c       evalate the differential of the rhs of the differential equation
c 
        do 1400 i=1,m
...
c 
c 
        subroutine stauadap(ier,n,ncorrect,a,b,phi0,funeva,
...
C          INVOLVE CHANGING THE SUBROUTINES STSOLVE1, STSOLVE2.
c 
c                       input parameters:
c 
...
c 
        ...
        maxiter=100 000
c 
        call stauada1(ier,n,a,b,phi0,funeva,funder,
...
c 
c 
        subroutine stauada1(ier,n,aaa,bbb,phi00,funeva,funder,
...
c 
        ...
        itype=1
        call stauinmt(n,ainte,adiff,ts,whts,endinter,
     1      itype,w,uuu)
c 
 1200 continue
c 
...
c       act accordingly
c 
        call stdisrep(uuu,n,m,phik1,ff,discr,val,cw1,cw2)
c 
...
c 
 ...
 1800 continue
c 
        call starrmo(phi11,phi0,m*2)
c 
        if(ifsave .eq. 0) goto 1900
        call starrmo(phik1,phiout(1,numpts+1),n*m*2)
        call starrmo(t,tout(numpts+1),n)
 1900 continue
c 
        numpts=numpts+n
        nmore=nmore+1
...
c 
c 
        subroutine stauexp(ier,n,n2,ncorr,ncorr2,a,b,phi0,
...
c 
     ...
     1      itype,w,uuu)
c 
        call stauinmt(n2,ainte2,adiff,ts2,whts,endinte2,
     1      itype,w,uuu2)
c 
 1200 continue
c 
...
c 
        ...
        if(b .ge. bbbb) step=b-a
c 
        call ststd(jer,ts2,n2,phi0,funeva,funder,par1,par2,par3,
...
c 
c 
        subroutine stdisrep(uuu,n,m,phik,w,discr,val,cw1,cw2)
        implicit real *8 (a-h,o-z)
        save
        dimension uuu(1)
...
c 
        ...
        w(i,j)=cw2(j)
 1800 continue
cccc         call prin2('in stdisrep, cw2=*',cw2,n*2)
...
c 
c 
        subroutine ststd(ier,ts,n,phi0,funeva,funder,
     ...
     3      ff,delta,rhs,cd,err,eps,cw1,cw2,nfcalls,nfinv)
        ...
        complex *16 phi0(m),phik(m,n),cd(1),cdd,
     1      phi1(m),ff(1),delta(1),rhs(1),cw1(1),cw2(1)
  
        complex *16 dermat(100 000),work(300),rhs2(300)
...
c                       work arrays:
c 
c  ff,delta,rhs - must be at least n*m+2 complex *16 locations long
c  cd,cw1,cw2 - must be at least m+2 complex *16 locations long
c 
...
c 
        call starrmo(phi0,phik(1,1),m*2)
c 
        do 1200 i=2,n
c 
        nfinv=nfinv+1
        nrefine=0
c 
        call stsolve1(funeva,funder,m,t(i)-t(i-1),phik(1,i-1),
     1      t(i),phik(1,i),
...
c 
        ...
        do 2000 ijk=1,ncorr
c 
        call stoneeul(ier,t,n,phik,ainte,phi0,
...
c 
c 
        subroutine stoneeul(ier,t,n,phik,ainte,phi0,
     1      funeva,u,h0,par1,par2,par3,m,ff,delta,rhs,a,
     2      nfcalls,nfinv,erreuler,funder,cd,cw1,cw2,
     3      dermat)
        implicit real *8 (a-h,o-z)
...
c 
        ...
        nrefine=0
c 
            call stsolve2(funeva,funder,m,h0,t(1),phik(1,1),
...
c 
        ...
        nrefine=0
c 
        call stsolve2(funeva,funder,m,hs(i-1),t(i),phik(1,i),
...
c 
c 
        subroutine stsolve1(funeva,funder,m,h,phik,tkp1,phikp1,
     1      dermat,work,rhs,pareva1,pareva2,pareva3,
     2      parder1,parder2,parder3,nrefine)
        implicit real *8 (a-h,o-z)
        save
        complex *16 phik(m),phikp1(m),dermat(m,m),work(1),
...
c                   work arrays:
c 
c  dermat - must be at least m**2 complex *16 locations
c  work, rhs - must be at least m complex *16 locations each
c 
...
cccc        call prin2('in stsolve1, parder3=*',parder3,m*2)
cccc        call prin2('in stsolve1, pareva3=*',pareva3,m*2)
...
c             it by h
c 
cccc        call prin2('calling funder from stsolve1, phikp1=*',
cccc     1      phikp1,m*2)
  
        call funder(tkp1,phikp1,dermat,m,parder1,parder2,parder3)
  
cccc        call prin2('in stsolve1, dermat=*',dermat,m*m*2)
...
c 
c 
        call starrmo(rhs,rhs2,m*2)
        call cgrmsol(dermat,m,rhs2,work,rcond)
...
c 
c 
        subroutine stsolve2(funeva,funder,m,h,tkp1,phikp1,
     1      dermat,delta,rhs,pareva1,pareva2,pareva3,
     2      parder1,parder2,parder3,nrefine)
        implicit real *8 (a-h,o-z)
        save
        complex *16 phikp1(m),dermat(m,m),delta(1),
...
c 
        call starrmo(rhs,rhs2,m*2)
        call cgrmsol(dermat,m,rhs2,delta,rcond)
c 
        return
 1700 continue
c 
...
c 
 ...
 2800 continue
        ...
        call cgrmsol(dermat,m,rhs2,delta2,rcond)
  
cccc        call prin2('after cgrmsol, delta2=*',delta2,m*2)
...
c 
c 
        subroutine stauinmt(n,ainte,adiff,x,whts,endinter,
     1      itype,w,u)
        implicit real *8 (a-h,o-z)
        save

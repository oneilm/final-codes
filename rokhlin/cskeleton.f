...
c        cskeleton, cskeleton3, cskel_basic, cskel_utv. Following
...
c 
c   cskel_basic - this is a simplified and accelerated version
c        of the subroutines cskeleton3, cskeleton. the integer
...
c 
c 
        subroutine cskel_basic(b,n,m,eps,irows,icols,ncols,w)
        implicit real *8 (a-h,o-z)
        save
        dimension rnorms(1),irows(1),icols(1)
        complex *16 b(n,m),w(1)
c 
...
c    CSKELETON3 (SEE). IT IS FASTER AND SIMPLER THAN EITHER
C    CSKELETON3 OR CSKELETON, BUT THE INTEGER VECTORS IROWS,
...
c 
        call cskel_piv2(b,n,m,eps,w,icols,ncols)
  
cccc        call prin2('in cskel_basic, first rnorms=*',w,ncols)
c 
c        transpose the compressed matrix
c 
        call cleastra(b,n,ncols,w)
c 
c        . . . gram-schmidt the columns of the transposed matrix
c 
        call cskel_piv(w,ncols,n,b,irows)
c 
cccc        call prin2('in cskel_basic, second rnorms=*',b,ncols)
c 
        return
c 
 2000 continue
...
c 
        call cleastra(b,n,m,w)
c 
c        . . . gram-schmidt the columns of the transposed matrix
c 
        call cskel_piv2(w,m,n,eps,b,irows,ncols)
  
cccc        call prin2('in cskel_basic, first rnorms=*',b,ncols)
c 
c        transpose the compressed matrix
c 
        call cleastra(w,m,ncols,b)
c 
        call cskel_piv(b,ncols,m,w,icols)
  
cccc        call prin2('in cskel_basic, second rnorms=*',w,ncols)
c 
        return
        end
...
c 
c 
        subroutine cskel_piv2(b,n,m,eps,rnorms,ipivots,ncols)
        implicit real *8 (a-h,o-z)
        save

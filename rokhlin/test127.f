...
c 
        call quabubb(xs,ys,ws,npts-1)
  
        istart=0
        do 2400 jj=1,npts
c 
        istart=istart+1
        if(ws(istart) .gt. 0) goto 2600
 2400 continue
 2600 continue
  
  
        jj=npts-istart
        call pntsignif(xs(istart),ys(istart),ws(istart),jj,
     1      nfuns,signifs,mmax)
  
        call prin2('and signifs after reordering*',signifs,jj)
c 
        call quabubb3(signifs,xs(istart),ys(istart),ws(istart),jj)
...
c 
        ...
        call ifinside(xs2(i),ys2(i),ifin)
  
cccc        if(ifin .eq. -1) ifout=1
...
c 
        ...
        call ifinsin(vert21(1),vert21(2),vert22(1),vert22(2),
     1      vert23(1),vert23(2) )
  
cccc        call ifinsin(vert1(1),vert1(2),vert2(1),vert2(2),
cccc     1      vert3(1),vert3(2) )

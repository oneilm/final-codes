        ...
        dimension w(1000 000),rlams(10 000), ipivots(10 000),
...
c 
        ...
        ifsvd=1
        call cmvecpsvd(ier,matfun2,par1,par2,par3,par4,par5,n,
     1    nvects,ifsvd,eps,ncols,rlams,lenrlams,ipivots,
     2    w,lenw,lused)
c 
        call prin2('after cmvecpsvd, rlams=*',rlams,ncols*2)
        call prinf('after cmvecpsvd, lused=*',lused,1)
...
c 
       ...
       do 2200 j=1,ncols
        ...
        call cmvecpsca(a(1,i),a(1,j),n,prods(j,i))
 2200 continue
c 
       call prinf('i=*',i,1)
...
c 
c 
        subroutine cmvecpsvd(ier,matfun,par1,par2,par3,par4,par5,
...
c       "significant" columns of the user-specified rectangular matrix
c 
...
c  ipivots - the integer array of length ncols, containing the
c            sequence of "significant" columns of a
c 
c                            Work array:
c 
c  w - must be big
...
c 
         call cmvecpcin(nopt,nblocks,nlast)
c 
...
c 
        ...
        lenw2=lenw-lrhs-2
c 
        call cmvecpcom(jer,matfun,par1,par2,par3,par4,par5,n,
     1    nblocks,maxblock,eps,rlams,w(irhs),lenrlams,
     2      ncols,ipivots,w(iw),lenw2,lused)
c 
        call prinf('in cmvecpsvd, ifsvd=*',ifsvd,1)
c 
...
c 
        call cmvecpmov(w(iw),w,ncols*n+2)
c 
        do 1500 i=1,ncols
c 
...
c 
        call prinf('in cmvecpsvd after cmvecpcom, ncols=*',
     1     ncols,1)
        call prin2('in cmvecpsvd after cmvecpcom, rlams=*',
     1     rlams,ncols*2)
c 
...
c 
c 
        subroutine cmvecpsmallma(phis,n,ncols,m,b,par1,par2,par3,
...
c 
        do 1800 j=1,ncols
c 
        call cmvecpsca(col,phis(1,j),n,prod)
c 
        b(j,i)=prod
 1800 continue
c 
...
c 
c 
        subroutine cmvecpcre(nummat,a,rhs,n,m,ipivots,matfun,
...
c 
 ...
 2000 continue
c 
cccc        call prin2('exiting cmvecpcre, a=*',a,n*m)
...
c 
c 
        entry cmvecpcin(nopt7,nblocks7,nlast7)
c 
        nopt=nopt7
        nblocks=nblocks7
        nlast=nlast7
...
c 
c 
        subroutine cmvecpcom(ier,matfun,par1,par2,par3,par4,
...
c        by a prior call to the subroutine cmvecpbox (see)
...
c 
        call cmvecpbox(nblocks,w(iboxes),nboxes)
c 
...
c 
        ...
        lenw2=lenw-iw-1
c 
        call cmvecprec(ier,matfun,par1,par2,par3,par4,par5,
...
c 
c 
        subroutine cmvecprec(ier,matfun,par1,par2,par3,
...
c 
 ...
 1400 continue
c 
        call cmvecprea(matfun,par1,par2,par3,par4,par5,nummat,
     1    eps,w(ia),w(irhs),n,m,w(irnorms),ncols,w(iipivots))
c 
        index=boxes(1,ibox)
        call cmvecpsto(ier,index,w(ia),w(irhs),n,ncols,w(irnorms),
     1      w(iipivots),w,lenw)
c 
        if(ier .ne. 0) return
c 
...
c 
        ...
        call cmvecppnt(ier,index1,ia1,irhs1,n1,ncols1,irnorms1,
     1     iipivots1,w,inuse)
c 
        index2=boxes(1,ison2)
        call cmvecppnt(ier,index2,ia2,irhs2,n,ncols2,irnorms2,
     1     iipivots2,w,inuse)
c 
        call prinf('before cmvecpmrg, inuse=*',inuse,1)
c 
        lbuff=mmax*2+mmax*n+100
c 
        irhs=inuse +2 +lbuff
...
c 
 ...
 3000 continue
c 
        call cmvecpmrg(w(ia1),ncols1,w(irnorms1),w(irhs1),w(iipivots1),
     1      w(ia2),ncols2,w(irnorms2),w(irhs2),w(iipivots2),n,eps,w(ia),
     2      ncols,w(irnorms),w(irhs),w(iipivots) )
c 
...
c 
        call cmvecpmov(w(irnorms),rnorms,ncols+1)
        call cmvecpmov(w(irhs),rhs,ncols+1)
        call cmvecpmovint(w(iipivots),ipivots,ncols+1)
        call cmvecpmov(w(ia),w,ncols*n+2)
  
         call prin2('exiting cmvecprec, w(irnorms=*',w(irnorms),
     1          ncols*2)
...
c 
c 
        subroutine cmvecpsto(ier,index,a,rhs,n,ncols,rnorms,ipivots,
     1     w,lenw)
        implicit complex *16 (a-h,o-z)
        save
...
c 
        call cmvecpmov(rnorms,w(irnorms),lrnorms)
        call cmvecpmov(rhs,w(irhs),lrhs)
        call cmvecpmov(ipivots,w(iipivots),lipivots)
        call cmvecpmov(a,w(ia),la)
c 
...
c 
c 
        entry cmvecpret(ier,index,a,rhs,n,ncols,rnorms,ipivots,w)
c 
...
c 
        call cmvecpmov(w(irnorms),rnorms,lrnorms)
        call cmvecpmov(w(irhs),rhs,lrhs)
        call cmvecpmov(w(iipivots),ipivots,lipivots)
        call cmvecpmov(w(ia),a,la)
c 
        return
c 
c 
...
c 
c 
        subroutine cmvecpmovint(ix,iy,n)
        implicit complex *16 (a-h,o-z)
        save
        dimension ix(1),iy(1)
c 
...
c 
c 
        subroutine cmvecprea(matfun,par1,par2,par3,par4,par5,
...
c 
        call cmvecpcre(nummat,a,rhs,n,m,ipivots,matfun,par1,par2,par3,
     1      par4,par5)
c 
...
c 
        ifpivot=1
        call cmvecpgrm(a,n,m,rnorms,eps,ncols,ifpivot,ipivots)
c 
ccc        call prinf('after cmvecpgrm, m=*',m,1)
ccc        call prinf('after cmvecpgrm, ncols=*',ncols,1)
ccc        call prinf('after cmvecpgrm, ipivots=*',ipivots,ncols)
c 
c 
        return
        end
...
c 
c 
        subroutine cmvecpbox(n,boxes,nboxes)
        implicit complex *16 (a-h,o-z)
        save
...
c 
        ...
        call cmvecpbdi(boxes(1,ibb),boxes(1,icurr),boxes(1,icurr+1),
     1      ifdiv,icurr)
c 
...
c 
c 
        subroutine cmvecpbdi(dad,son1,son2,ifdiv,icurr)
        implicit complex *16 (a-h,o-z)
        save
...
c 
c 
        subroutine cmvecpmrg(a,ncolsa,rnormsa,rhsa,ipivotsa,
...
c 
        ncc=ncolsa+ncolsb
        ifpivot=1
        call cmvecpgrm(c,n,ncc,rnormsc,eps,ncolsc,ifpivot,ipivotsc)
c 
ccc        call prinf('after cmvecpgrm, ncc=*',ncc,1)
ccc        call prinf('after cmvecpgrm, ncolsc=*',ncolsc,1)
ccc        call prinf('after cmvecpgrm, ipivotsc=*',ipivotsc,ncolsc)
c 
        return
        end
c 
...
c 
c 
        subroutine cmvecpudv(a,n,u,eps,numrows,nn)
        implicit complex *16 (a-h,o-z)
        save
...
c 
        ...
        b(2,1)=a(j,i)
        b(2,2)=a(j,j)
        call cmvecp4uv(ier,b,rlam,u0,v0)
c 
...
c 
 ...
 4200 continue
c 
        call cmvecpsrt(a(1,1),n,u,v,a(1,2),ifuv,nn)
c 
        return
        end
c 
...
c 
c 
        subroutine cmvecpsrt(s,n,u,v,iw,ifuv,nn)
...
c 
c 
        subroutine cmvecp4uv(ier,a0,rlam,u,v)
        implicit real *8 (a-h,o-z)
        save
...
c 
         d=dsqrt(d1)
         call cmvecpd2or(v(1,1),v(1,2))
         goto 3200
c 
 3000 continue
c 
         d=dsqrt(d2)
         call cmvecpd2or(v(1,2),v(1,1))
 3200 continue
c 
c       obtain the diagonal matrix
c 
         call cmvecpd4prodc(a0,uu,w1)
         call cmvcpcon(v,b)
         call cmvecpd4prodc(b,w1,w3)
c 
        call cmvecpcopy2(w2,u)
c 
...
c 
c 
        subroutine cmvecpd4prodc(a,b,c)
        implicit complex *16 (a-h,o-z)
        save
...
c 
c 
        subroutine cmvecpgrm(b,n,m,rnorms,eps,ncols,ifpivot,ipivots)
        implicit real *8 (a-h,o-z)
        save
        complex *16 b(n,m),cd
        real *8 rnorms(1)
...
c 
        ...
        do 2780 j=1,i-1
c 
        call cmvecpsca(b(1,i),b(1,j),n,cd)
c 
cccc        cd=dconjg(cd)
        do 2770 l=1,n
...
c 
        call cmvecpsca(b(1,i),b(1,i),n,cd)
c 
...
c 
c 
        subroutine cmvecpsca(x,y,n,prod)
        implicit complex *16 (a-h,o-z)
        save
        dimension x(1),y(1)
c 

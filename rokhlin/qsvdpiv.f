...
c 
        ...
        implicit real *16 (a-h,o-z)
        save
        dimension a(n,m),u(n,m),v(m,n),w(1),s(2000),
...
c 
        ...
        do 1200 j=1,n
        a(j,i)=i**2-j**2
        a(j,i)=qexp(-tt(i)*tt(j))+qexp(-tt(i)/tt(j))
 1200 continue
 1400 continue
c 
c      copy a into a2
...
c 
cccc        t1=second(dd)
        call qsvdpiv(ier,a,n,m,u,v,s,ncols,eps,
     1      w,lw,ltot)
cccc        t2=second(dd)
cccc        call prinq('and time for the SVD is*',t2-t1,1)
        call prinf('after qsvdpiv, ier=*',ier,1)
        call prinf('after qsvdpiv, ncols=*',ncols,1)
c 
c       test the accuracy
c 
        call mulback(u,w1,v,n,ncols,m,a3,w2,s)
ccc        call prinq('a3=*',a3,n*m)
ccc        call prinq('while a2=*',a2,n*m)
c 
        err=0
        do 2200 i=1,m
        do 2000 j=1,n
 ...
 2000 continue
 2200 continue
        call prinq('and the error is*',qsqrt(d),1)
...
c 
c 
        subroutine qsvdpiv(ier,a,n,m,u,v,s,ncols,eps,
     1      w,lw,ltot)
        implicit real *16 (a-h,o-z)
        save
...
c  ltot - the total amount of space (in terms of real *16 words)
...
c 
        ifpivot=1
        call qsvdpiv0(a,n,m,u,v,ncols,w,eps,ifpivot)
cccc         call prinq('after first qsvdpiv0, rnorms=*',w,ncols)
c 
...
c 
        iffirst=0
        call qsvdpiv1(a,u,v,w(iw),w(it),n,m,ncols,
     1    w(irnorms),eps,w(iv2),w(iu2),w(iww),iffirst)
cccc         call prinq('after qsvdpiv1, w(it)=*',w(it),ncols)
c 
...
c 
c 
        subroutine qsvdpiv1(a,u,v,w,t,n,m,ncols,rnorms,
...
c 
        ...
        ifpivot=1
        call qsvdpiv0(a,n,m,u,v,ncols,rnorms,eps2,ifpivot)
cccc         call prinq('after first qsvdpiv0, rnorms=*',rnorms,ncols)
 1200 continue
c 
...
c 
        ifpivot=0
        call qsvdpiv0(v,m,ncols,w,t,ncols2,rnorms,eps2,ifpivot)
cccc         call prinq('after second qsvdpiv0, rnorms=*',rnorms,ncols)
c 
c        use jacobi rotations to sv-decompose the matrix t
c 
        ifuv=1
        call qd2mudv(t,ncols,u2,v2,eps2,ifuv,numrows)
        call prinq('after qd2mudv, t=*',t,ncols)
        call prinf('after qd2mudv, numrows=*',numrows,1)
c 
c       multiply the matrix u from the right by u2, and the
c       matrix w from the right by v2^*
c 
        call qsvdfinm(u,u2,w,v2,n,m,ncols,ww)
        return
        end
c 
c 
...
c 
c 
        subroutine qsvdfinm(u,u2,w,v2,n,m,ncols,ww)
        implicit real *16 (a-h,o-z)
        save
...
c 
c 
        subroutine qsvdpiv0(a,n,m,b,v,ncols,rnorms,eps,ifpivot)
        implicit real *16 (a-h,o-z)
        save
...
c 
         call qsvdgrm(b,n,m,rnorms,eps,ncols,ifpivot)
c 
...
c 
        ...
        do 2200 j=1,m
c 
        call qsvdscap(a(1,j),b(1,i),n,prod)
        v(j,i)=prod
 2200 continue
 2400 continue
        return
...
c 
c 
        subroutine qsvdgrm(b,n,m,rnorms,eps,ncols,ifpivot)
        implicit real *16 (a-h,o-z)
        save
...
c 
c 
        subroutine qsvdscap(x,y,n,prod)
        implicit real *16 (a-h,o-z)
        save
        dimension x(1),y(1)
c 
...
c 
c 
        subroutine qd2mudv(a,n,u,v,eps,ifuv,numrows)
        implicit real *16 (a-h,o-z)
        save
...
c 
        ...
        do 1800 j=1,n
         if(j .eq. i) goto 1800
         if( (qabs(a(i,j)) .lt. thresh) .and.
     1       (qabs(a(j,i)) .lt. thresh) ) goto 1800
        ifany=1
        numrows=numrows+1
c 
...
c 
 ...
 4200 continue
c 
       call qd2msvsr(a(1,1),n,u,v,a(1,2),ifuv)
        return
        end
c 
c 
...
c 
c 
        subroutine qd2msvsr(s,n,u,v,iw,ifuv)
        implicit real *16 (a-h,o-z)
        save
...
c 
        if(qabs(den) .lt. eps*qabs(rnum)) goto 1200
        tgphi=rnum/den
        phi=qatan(tgphi)
cccc        alpha=qcos(phi)
cccc        beta=qsin(phi)
        call foolcosi(phi,alpha,beta)
        goto 1400
 1200 continue
c 
...
c       if denominator is not too small, use exact formulae
c 
        if(qabs(den) .lt. eps*qabs(rnum)) goto 1600
        tg2phi=-rnum/den
        phi=qatan(tg2phi)/2
cccc        alpha=qcos(phi)
cccc        beta=qsin(phi)
...
c 
        alpha=qsqrt((1-den/rnum)/2)
        beta=qsqrt((1+den/rnum)/2)
 1800 continue
c 

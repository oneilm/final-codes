        ...
        dimension xs(10 000),ws(10 000),rlams3(10 000),diffs(10 000),
     1      w(40 000 000),rlams32(10 000)
c 
        call prini(6,13)
C 
...
cccc        time1=clotatim()
        ...
        call proquadr(ier,c,eps,ifrefine,ifeven,npts,xs,ws,
     1      rlams3,nvects,rlams32,nvects2,w,lenw,lused)
  
cccc        time2=clotatim()
  
  
cccc        call prin2('and time for proquadr is*',time2-time1,1)
  
cccc        stop
  
        call prinf('and after proquadr, npts=*',npts,1)
        call prinf('and after proquadr, lused=*',lused,1)
        call prinf('and after proquadr, ier=*',ier,1)
        call prin2('and xs=*',xs,npts)
        call prin2('and ws=*',ws,npts)
  
        if(ier .ne. 0) stop
c 
        call prin2('after proquadr, rlams32 =*',rlams32,nvects2)
...
c 
 ...
 1200 continue
        ...
        call prin2('and distances between nodes are*',diffs,npts-1)
c 
c       test the obtained quadrature
c 
        ntest=1000
        cc=c
        call quatest(cc,ntest,xs,ws,npts,errmax)
c 
c       store the obtained quadrature on disk
c 
        iw=17
  
        call quadwrt(iw,xs,ws,npts,c,eps,rlams3(2*npts+1),errmax)
  

        rat=diffs(npts/2)/diffs(1)

        call prin2('and rat=*',rat,1)  

        done=1
        call proleva(npts*2,done,w,val,der)
cccc        call prolevv(1,done,w,val,der)

        call prin2('and val=*',val,1)
        call prin2('and der=*',der,1)


        call prin2('and xs(1)+1=*',xs(1)+1,1)
...
c 
c 
        subroutine quadwrt(iw,roots,whts,npts,c,eps,rlam,errmax)
        implicit real *8 (a-h,o-z)
        save
        dimension roots(1),whts(1)
c 
c       store on disk the nodes of the quadrature
c 
        numwrt=(npts+1)/2
  
        call prinf('in quadwrt, numwrt=*',numwrt,1)
        call prinf('in quadwrt, npts=*',npts,1)
  
        nlines=(numwrt+1)/2
c 
  
        call prinf('in quadwrt, nlines=*',nlines,1)
  
  
 1050 format(10x,/,'c     ','In this run, c=',e22.16,', ',
     1    'eps=',e22.16,',',/,'c     ','npts=',i6,
     2    ' and expected error of quadrature is ',e11.5,/,
     3    'c      and maximum error of quadrature is ',e11.5)
  
 1060 format(10x,/,'c     ','Please note that only the first ',
     1  'half of the nodes ',/,
     2    'c     (those that are \leq 0) are output',/,10x)
  
        write(iw,1050) c,eps,npts,rlam,errmax
        write(iw,1060)
  
  
 1100 format('c        Nodes are',/,10x)
        write(iw,1100)
...
c 
        ...
        if(i*2 .le. numwrt)
     1      write(iw,1200) j,roots(i*2-1),roots(i*2)
c 
        if(i*2 .gt. numwrt)
     1      write(iw,1200) j,roots(i*2-1)
c 
 1200 format(5x,i1,2x,e22.16,',',e22.16,',')
 1400 continue
c 
  
       call prin2('whts=*',whts,npts)
  
 2100 format(10x,/,'c        Weights are',/,10x)
        write(iw,2100)
...
c 
        ...
        if(i*2 .le. numwrt)
     1       write(iw,2200) j,whts(i*2-1),whts(i*2)
c 
        if(i*2 .gt. numwrt)
     1      write(iw,2200) j,whts(i*2-1)
c 
 2200 format(5x,i1,2x,e22.16,',',e22.16,',')
 2400 continue
c 
        return
        end
...
c 
c 
        subroutine quatest(c,ntest,xs,ws,npts,errmax)
        implicit real *8 (a-h,o-z)
        save
        complex *16 ima,cints(10 000),cints2(10 000)
        dimension cs(10 000),xs(1),ws(1),diffs(10 000)
c 
        data ima/(0.0d0,1.0d0)/
c 
c       discretize the space of exponetials
c 
        hcs=c/ntest
        do 1200 i=1,ntest
c 
 ...
 1200 continue
c 
cccccc        call prin2('cs=*',cs,ntest)
c 
c       one exponential after another, evaluate their integrals
c       on the interval [-1,1] analytically
c 
        do 1600 i=1,ntest
c 
        cints(i)=1/(ima*cs(i))*(exp(ima*cs(i))-exp(-ima*cs(i)))
 1600 continue
c 
cccc        call prin2('integrals evaluated analytically*',
cccc     1      cints,ntest*2)
c 
...
c 
        cd=0
        do 2200 j=1,npts
        cd=cd+ws(j)*exp(ima*cs(i)*xs(j))
 2200 continue
c 
        cints2(i)=cd
        diffs(i)=abs(cints2(i)-cints(i))
  
        if(errmax .lt. diffs(i)) errmax=diffs(i)
 2400 continue
c 
cccc        call prin2('and integrals evaluated numerically*',
cccc     1      cints2,ntest*2)
c 
  
cccc        call prin2('and absolute values of differences are*',
cccc     1         diffs,ntest)

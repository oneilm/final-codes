        ...
        external fun
        dimension vals(100),traps(100),rints(100),work(100 000)
c 
        call prini(6,13)
C 
...
c 
        ...
        eps=1.0d-15
c 
        call adapgaum(ier,a,b,fun,nn,jtest,par2,m,eps,
     1      rints,maxrec,numint,work)
        call prin2('after adapgaum, rints=*',rints,nn)
        call prinf('after adapgaum, ier=*',ier,1)
        call prinf('after adapgaum, maxrec=*',maxrec,1)
        call prinf('after adapgaum, numint=*',numint,1)
...
c 
         ...
         val=dlog(x)+dlog(1-x)+dlog(dabs(x-half))
cccc         fun=dlog(dabs(x))+dlog(dabs(done-x))
cccc          fun=half/x
  
cccc         call prinf('inside fun, n=*',n,1)
...
c 
c 
        subroutine adapgaum(ier,a,b,fun,n,par1,par2,m,eps,
     1      rint,maxrec,numint,work)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        lvalue3=n+10
c 
        ivals=ivalue3+lvalue3
c 
        call adinrecm(ier,stack,a,b,fun,n,par1,par2,t,w,m,
     1      work(ivals),nnmax,eps,rint,maxdepth,maxrec,
     2      numint,work(ivalue2),work(ivalue3) )
        return
        end
c 
c 
...
c 
     ...
     2      rints,maxdepth,maxrec,numint,value2,value3)
        implicit real *8 (a-h,o-z)
        save
        dimension stack(2,1),t(1),w(1),vals(n,1),par1(1),par2(1),
     1      value2(1),value3(1),rints(1)
c 
c       start the recursion
c 
        stack(1,1)=a
        stack(2,1)=b
        call oneintmu(a,b,fun,n,par1,par2,t,w,m,vals(1,1))
c 
...
c 
        ...
        call oneintmu(stack(1,j),c,fun,n,
     1      par1,par2,t,w,m,value2)
c 
        call oneintmu(c,stack(2,j),fun,n,
...
c 
        ...
        call arrmomom(value2,vals(1,j+1),n)
c 
        stack(1,j)=(stack(1,j)+stack(2,j))/2
        call arrmomom(value3,vals(1,j),n)
c 
        j=j+1
c 
...
c 
c 
        subroutine oneintmu(a,b,fun,n,par1,par2,t,w,m,rints)
        implicit real *8 (a-h,o-z)
        save
        dimension t(1),w(1),par1(1),par2(1),rints(1),vals(1000)
c 

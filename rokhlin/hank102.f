...
c       and hank102, and compare them things
c 
        err0max=0
        err1max=0
...
c        for an arbitrary user-specified complex number z. This is
c        a slightly accelerated and prettified version of the old
c        hank101, whose principal claim to fame is that it is valid
c        on the whole complex plane, and is reasonably accurate
c        (14-digit relative accuracy) and reasonably fast.
c 
c                      input parameters:
c 
...
c 
        ...
        zr=-zu
c 
        call hank102u(zu,ier,h0u,h1u)
        call hank102r(zr,ier,h0r,h1r)
c 
...
c 
c 
        subroutine hank102u(z,ier,h0,h1)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        zzz9=z**9
        m=35
        call hank102p(c0p1,m,cd,h0)
        h0=h0*ccex * zzz9
c 
        call hank102p(c1p1,m,cd,h1)
        h1=h1*ccex * zzz9
        return
 2000 continue
c 
...
c 
        ...
        ccex=cdexp(ima*z)*cd
        m=31
        call hank102p(c0p2,m,cd,h0)
        h0=h0*ccex
c 
        m=31
        call hank102p(c1p2,m,cd,h1)
        h1=h1*ccex
        return
 3000 continue
c 
...
c 
c 
        subroutine hank102r(z,ier,h0,h1)
        implicit real *8 (a-h,o-z)
        save

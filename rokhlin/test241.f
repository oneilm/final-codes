...
c 
        ...
        call projfilt(ier,fin,n,cm,sgain,ifsecant,fout,
     1      ff,ff2,w,ifinit)
c 
...
c 
        ...
        call projfilt(ier,fout,n,cm,sgain2,ifsecant,fout2,
     1      ff,ff2,w,ifinit)
...
c 
c 
        subroutine projfilt(ier,fin,n,cm,sgain,ifsecant,fout,
     1      ff,ff2,w,ifinit)
        implicit real *8 (a-h,o-z)
        save
...
c  sgain - the supergain the subroutine will
...
c 
        ...
        call projflt(ier,fin,n,cm,sgain,ifsecant,fout,
...
c 
c 
        subroutine projflt(ier,fin,n,cm,sgain,ifsecant,fout,
...
c 
        call dersall(jer,cm2,prods,n,totpowr,
...
c       be enforced approximately - bypass the secant process
c 
        if(ifsecant .eq. 0) goto 3200
        if(ier .ne. 0) goto 3200
c 
c       if the user so requested, use secant iterations
...
c 
        ...
        call dersall(ier,cm2,prods,n,totpowr3,
     1      rnum,denom,rr,drrds,rlout,w)
c 
...
c 
        call lesqarrm(ff2,fout,n*2)
c 
        call DCFFTF(N,fout,WSAVE)
c 
        return
        end
c 
...
c 
c 
        subroutine dersall(ier,cm,prods,n,s,
...
c 
        call dersall0(ier,cm,prods,n,s,w(ialph2),w(ialph2d),
...
c 
c 
        subroutine dersall0(ier,cm,prods,n,s,alph2,alph2der,
...
c 
        call lesqbis(ier,cm,prods,n,s,rl)
c 
        if(ier .eq. 16) return
c 
c       evaluate the derivative of S with respect to rl
c 
        call lesqder(cm,prods,n,rl,dsdrl)
c 
...
c 
c 
        subroutine lesqbis(ier,cm,prods,n,totpowr,rlout)
        implicit real *8 (a-h,o-z)
        save
...
c 
        ...
        rlout=0
c 
        call lesqsum(cm,prods,n,rl1,f1)
        call lesqsum(cm,prods,n,rl2,f2)
c 
...
c 
        rl22=rl2/10
c 
        call lesqsum(cm,prods,n,rl22,f22)
c 
...
c 
        rl3=(rl1+rl2)/2
c 
        call lesqsum(cm,prods,n,rl3,f3)
c 
...
c 
        do 1600 i=1,5
c 
        call lesqsum(cm,prods,n,rlout,sum)
        call lesqder(cm,prods,n,rlout,der)
c 
        ff=sum-totpowr
c 
        rlout=rlout-ff/der
...
c 
c 
        subroutine lesqder(cm,prods,n,rlagr,der)
        implicit real *8 (a-h,o-z)
        save
        dimension cm(1)
        real *8 prods(1)
...
c 
c 
        subroutine lesqsum(cm,prods,n,rlagr,sum)
        implicit real *8 (a-h,o-z)
        save
        dimension cm(1)
        real *8 prods(1)
